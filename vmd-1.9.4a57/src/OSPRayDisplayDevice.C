...
* RCS INFORMATION:
*
*      $RCSfile: OSPRayDisplayDevice.C,v $
*      $Author: johns $      $Locker:  $               $State: Exp $
*      $Revision: 1.19 $         $Date: 2021/12/21 22:59:38 $
*
***************************************************************************/
/**
 *  \file OSPRayDisplayDevice.C
...
int OSPRayDisplayDevice::OSPRay_Global_Init(void) {
  return OSPRayRenderer::OSPRay_Global_Init(); // call only ONCE
}

// Global OSPRay shutdown routine -- call it only ONCE...
void OSPRayDisplayDevice::OSPRay_Global_Shutdown(void) {
  OSPRayRenderer::OSPRay_Global_Shutdown(); // call only ONCE
}
...
void OSPRayDisplayDevice::trimesh_c4u_n3b_v3f(unsigned char *c, signed char *n,
...
void OSPRayDisplayDevice::trimesh_n3b_v3f(signed char *n, float *v, int numfacets) {
  add_material();
...
void OSPRayDisplayDevice::start_clipgroup(void) {
  int i;
  int planesenabled = 0;
...
void OSPRayDisplayDevice::end_clipgroup(void) {
  if (inclipgroup) {
