 ...
 *      $Revision: 1.29 $      $Date: 2020/10/22 03:43:24 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
int VolumeTexture::allocateTextureMap(ptrdiff_t ntexels) {
  texid = 0;
  if (texmap) {
    vmd_dealloc(texmap);
    texmap = NULL;
  }
  ptrdiff_t sz = ntexels*3L*sizeof(unsigned char);
...
void VolumeTexture::generatePosTexture() {
  ...
  int x, y, z;
  ptrdiff_t addr, addr2;
  ptrdiff_t num = num_texels();

  if (!allocateTextureMap(num)) return;

  ptrdiff_t planesz = size[0] * size[1];
  for (z=0; z<size[2]; z++) {
    for (y=0; y<size[1]; y++) {
      addr = z * planesz + y * size[0];
      for (x=0; x<size[0]; x++) {
        addr2 = (addr + x) * 3L;
...
void VolumeTexture::generateIndexTexture() {
  ...
  size[0] = size[1] = size[2] = 32; 
  int x, y, z;
  ptrdiff_t addr, addr2, addr3, index;
  ptrdiff_t num = num_texels();
  ...
  for (z=0; z<size[2]; z++) {
    for (y=0; y<size[1]; y++) {
      ...
      for (x=0; x<size[0]; x++) {
        index = addr + x;
        addr2 = index * 3L;
        addr3 = ((int) ((index / (float) num) * 4095)) * 3L;
...
void VolumeTexture::generateChargeTexture(float vmin, float vmax) {
  ...
  ptrdiff_t planesz = size[0] * size[1];
  ptrdiff_t vplanesz = v->xsize * v->ysize;
  for (z=0; z<v->zsize; z++) {
    for (y=0; y<v->ysize; y++) {
      addr = z * planesz + y * size[0];
      daddr = z * vplanesz + y * v->xsize;
      for (x=0; x<v->xsize; x++) {
        addr2 = (addr + x) * 3L;
        float level, r, g, b;
...
void VolumeTexture::generateHSVTexture(float vmin, float vmax) {
  int x, y, z;
  ptrdiff_t index, addr, addr2, addr3;
  ptrdiff_t daddr;
  float vscale, vrange;
  unsigned char coltable[3 * 4096];
...
void VolumeTexture::generateColorScaleTexture(float vmin, float vmax, const Scene *scene) {
  ...
  // build an unsigned byte RGB color table to speed up conversion
  // by eliminating floating point to unsigned char conversions in the
  // innermost loops over voxels
  unsigned char * rgb3ub = (unsigned char *) calloc(1, MAPCLRS * 3 * sizeof(unsigned char)); 
  for (i=0; i<MAPCLRS; i++) {
    const float *rgb = scene->color_value(MAPCOLOR(i));
    int idx = i*3;
    rgb3ub[idx + 0] = (unsigned char)(rgb[0]*255.0f);
    rgb3ub[idx + 1] = (unsigned char)(rgb[1]*255.0f);
    rgb3ub[idx + 2] = (unsigned char)(rgb[2]*255.0f);
  }

  // map volume data scalars to colors
  ptrdiff_t planesz = size[0] * size[1];
  ptrdiff_t vplanesz = v->xsize * v->ysize;
  const int maxcolorindex = MAPCLRS-1;
  const float colscale = vscale * maxcolorindex;
  for (z=0; z<v->zsize; z++) {
    for (y=0; y<v->ysize; y++) {
      addr = (z * planesz + y * size[0]) * 3L;
      daddr = z * vplanesz + y * v->xsize;
      const float *data = &v->data[daddr];    // simplify inner loop ptr arith
      unsigned char *tmaprow = &texmap[addr]; // simplify inner loop ptr arith
      for (x=0; x<v->xsize; x++,tmaprow+=3) {
        ...
        int colindex = (int) ((data[x] - vmin) * colscale); 
        ...
        // clamp integer color index to 0..maxcolorindex
#if 1
        colindex = (colindex < 0) ? 0 : colindex;
        colindex = (colindex > maxcolorindex) ? maxcolorindex : colindex;
#elif 1
        int mask = 0 - (colindex > maxcolorindex);
        colindex = (maxcolorindex & mask) | (colindex & ~mask); 
#else
        if (colindex < 0) 
          colindex = 0;
        else if (colindex > maxcolorindex) 
          colindex = maxcolorindex;
#endif

        colindex *= 3; // compute offset in color table
        tmaprow[0] = rgb3ub[colindex    ];
        tmaprow[1] = rgb3ub[colindex + 1];
        tmaprow[2] = rgb3ub[colindex + 2];
      }
    }
  }
  ...
  double t = wkf_timer_time(timer);
  if (t > 5.0) 
    msgInfo << "Texture generation time: " << t << " sec." << sendmsg;
