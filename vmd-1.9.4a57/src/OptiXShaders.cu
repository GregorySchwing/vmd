...
*      $Revision: 1.176 $         $Date: 2021/05/14 22:15:01 $
*
***************************************************************************/
/**
 *  \file OptiXShaders.cu
 *  \brief Core OptiX/CUDA ray tracing engine implementation.
 * 
...
// XXX special shader modifications used in the VMD renderings
// included in the NSF CADENS "Birth of Planet Earth" movie
// #define CADENSMOVIE 1
...
rtDeclareVariable(unsigned int, max_trans, , );
...
struct PerRayData_radiance {
  ...
  unsigned int transcnt; // transmission ray surface count/depth
};
...
#if defined(ORT_RAYSTATS)
  raystats1_buffer[launch_index].x+=aa_samples; // increment primary ray counter
#endif
...
#if defined(CADENSMOVIE)
  // For planetarium dome renderings, use spherical distance rather than planar
  // XXX this should be a runtime parameter eventually
  float r = t_hit;
#else
...
static __device__ float3 shade_ambient_occlusion(float3 hit, float3 N, float aoimportance) {
  ...
  for (int s=0; s<nsamples; s++) {
    ...
    // NOTE: There are no safety checks for maximum RT recursion depth
    //       here, so we must have asked OptiX for a max recursion depth
    //       that is one greater than we're tracking elsewhere internally.
    rtTrace(root_shadower, ambray, shadow_prd);
    inten += ndotambl * shadow_prd.attenuation;
  } 
#if defined(ORT_RAYSTATS)
  raystats1_buffer[launch_index].z+=ao_samples; // increment AO shadow ray counter
#endif
...
#if defined(CADENSMOVIE)
  // XXX specially modified shading of transparent surfaces used to 
  //     give "cloud like" appearances to molecular surfaces 
  if ((p_transmode>1 || (TRANSMISSION_ON || CLIP_VIEW_ON)) && alpha < 0.999f ) {
    // Emulate Tachyon/Raster3D's angle-dependent surface opacity if enabled
    switch(int(p_transmode)) {

    case 3: // transmode=3 ==> alternate angle-modulated-transparency - try to not let it be fully opaque at edges
        alpha = 1.0f + cosf(3.1415926f * (1.0f-alpha) * dot(N, ray.direction));
        alpha = alpha*alpha * 0.165;  // something less than 0.25
        break;

    case 2: {
        // Glow case.  Why can't we make this be case 2, and set our special material's transmode to 2??
        float d = fminf( dot(N, ray.direction), 0.5f );  // flat-topped glow
        float v = alpha * fabsf(d*d); // - 0.5f;
        result = 3.5f * v * result; // * make_float3( v,v,v );
        alpha = v*0.15;
        // alpha = v*v*0.25f;  
        // result = make_float3( alpha, p_transmode*(0.5f - fabsf(d*d)), 0 );
        break;
      }

    case 1:
        // normal "angle-modulated transparency" case
        alpha = 1.0f + cosf(3.1415926f * (1.0f-alpha) * dot(N, ray.direction));
...
RT_PROGRAM void any_hit_opaque() {
  ...
  // upon hitting _any_ opaque object during an anyhit traversal, 
  // we should immediately terminate traversal
  rtTerminateRay();
}
