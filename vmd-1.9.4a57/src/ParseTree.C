 ...
 *  $Revision: 1.153 $		$Date: 2020/07/24 06:04:03 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
///////////////// the ParseTree
ParseTree::ParseTree(VMDApp *vmdapp, SymbolTable *parser, atomparser_node *parse_tree)
{
  app = vmdapp;
  tree = parse_tree;
  table = parser;
...
symbol_data * ParseTree::eval_mathop(atomparser_node *node, int num, int *flgs)
{
  ...
  int firstsel = 0;
  int lastsel = -1;
  if (!analyze_selection_aligned_dispatch(app->cpucaps, num, flgs, &firstsel, &lastsel, NULL)) {
...
static void same_int(VMDApp *app, symbol_data *tmp, symbol_data *tmp2, int num, 
                     int *subselect, int *flgs) {
  int firstsubsel = -1, lastsubsel = -1;

  if (analyze_selection_aligned_dispatch(app->cpucaps, num, subselect, &firstsubsel, &lastsubsel, NULL)) {
...
static void same_double(VMDApp *app, symbol_data *tmp, symbol_data *tmp2, 
                        int num, int *subselect, int *flgs) {
  int firstsubsel, lastsubsel, subselselected;
  if (analyze_selection_aligned_dispatch(app->cpucaps, num, subselect,
...
void ParseTree::eval_same(atomparser_node *node, int num, int *flgs) {
   ...
   switch (has_type) {
    case SymbolTableElement::IS_INT:   
      ...
      same_int(app, tmp, tmp2, num, subselect, flgs);
      break;
    ...
    case SymbolTableElement::IS_FLOAT: 
      ...
      same_double(app, tmp, tmp2, num, subselect, flgs);
      break;
