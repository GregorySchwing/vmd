 ...
 *      $Revision: 1.10 $      $Date: 2020/10/22 03:43:24 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
private:
  ...
  ptrdiff_t num_texels(void) { return ptrdiff_t(size[0])*ptrdiff_t(size[1])*ptrdiff_t(size[2]); }

  // allocate texture memory for n texels (3n bytes), and update texid.  
  // Return success.
  int allocateTextureMap(ptrdiff_t ntexels);
};
