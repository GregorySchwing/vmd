 ...
 *	$Revision: 1.374 $	$Date: 2021/10/28 21:12:15 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
    ...
    switch (atomRep->method()) {
      ...
      case AtomRep::FIELDLINES:
        ...
        draw_volume_field_lines((int)atomRep->get_data(AtomRep::SPHERERES),
          int(atomRep->get_data(AtomRep::FIELDLINESEEDUSEGRID)),
          ...
          int(atomRep->get_data(AtomRep::FIELDLINESTYLE)),
...
void DrawMolItem::draw_lines(float *framepos, int thickness, float cutoff) {
  ...
  sprintf(commentBuffer, "Mol[%d] Rep[%d] Lines", mol->id(), repNumber);
...
void DrawMolItem::draw_solid_cubes(float * framepos, float radscale) {
  ...
  sprintf(commentBuffer,"Mol[%d] Rep[%d] LatticeCubes", mol->id(), repNumber);
  cmdCommentX.putdata(commentBuffer, cmdList);
...
void DrawMolItem::draw_solid_spheres(float * framepos, int res, 
  ...
  sprintf(commentBuffer,"Mol[%d] Rep[%d] VDW", mol->id(), repNumber);
  cmdCommentX.putdata(commentBuffer, cmdList);
  append(DMATERIALON); // turn on lighting
...
void DrawMolItem::draw_residue_beads(float * framepos, int sres, float radscale) {
  ...
  // set sphere type, resolution, and shading characteristics
  sprintf(commentBuffer,"Mol[%d] Rep[%d] Beads", mol->id(), repNumber);
  cmdCommentX.putdata(commentBuffer, cmdList);
  append(DMATERIALON); // turn on lighting
  ...
  for (resid=0; resid<numres; resid++) {
    ...
    pickpointindices.append(pickindex);
  }
...
void DrawMolItem::draw_dotted_spheres(float * framepos, float srad, int sres) {
  ...
  // set sphere type, resolution, and shading characteristics
  sprintf(commentBuffer,"Mol[%d] Rep[%d] Dotted", mol->id(), repNumber);
  cmdCommentX.putdata(commentBuffer, cmdList);
...
void DrawMolItem::draw_points(float *framepos, float pointsize) {
  sprintf(commentBuffer, "Mol[%d] Rep[%d] Points", mol->id(), repNumber);
  cmdCommentX.putdata(commentBuffer, cmdList);
  append(DMATERIALOFF);
...
void DrawMolItem::draw_cpk_licorice(float *framepos, int cpk, float brad, int bres, float srad, int sres, int linethickness, float cutoff) {
  ...
  if (cpk) { 
    ...
    sprintf(commentBuffer, "Mol[%d] Rep[%d] Licorice", mol->id(), repNumber);
    cmdCommentX.putdata(commentBuffer, cmdList);
  ...
  sprintf(commentBuffer,"Mol[%d] Rep[%d] CPK", mol->id(), repNumber);
  cmdCommentX.putdata(commentBuffer, cmdList);
  ...
  if (use_bonds) {
    ...
    ResizeArray <float> cylpoints;
    ResizeArray <float> cylradii;
    ResizeArray <float> cylcolors;
#endif
...
#if defined(VMDOPTIXRTRT)
            cylpoints.append3(fp1);
            cylpoints.append3(mid);
            cylradii.append(brad);
            cylcolors.append3(scene->color_value(atomColor->color[i]));
#else
            // XXX here we should be aggregating into a cyl array primitive
...
#if defined(VMDOPTIXRTRT)
    DispCmdCylinderArray cmdCylinderArray;
    cmdCylinderArray.putdata(&cylpoints[0], &cylradii[0], &cylcolors[0], 
                             cylradii.num(), bres, 0, cmdList);
#endif
  }
...
void DrawMolItem::draw_bonds(float *framepos, float brad, int bres, float cutoff) {
  ...
  sprintf(commentBuffer,"Mol[%d] Rep[%d] Bonds", mol->id(), repNumber);
  cmdCommentX.putdata(commentBuffer, cmdList);
...
void DrawMolItem::draw_tube(float *framepos, float b_rad, int b_res) {
  ...
  sprintf(commentBuffer,"Mol[%d] Rep[%d] Tube", mol->id(), repNumber);
  cmdCommentX.putdata(commentBuffer, cmdList);
...
void DrawMolItem::draw_hbonds(float *framepos, float maxangle, int thickness, float maxdist) {
  ...
  sprintf(commentBuffer,"Mol[%d] Rep[%d] HBonds", mol->id(), repNumber);
  cmdCommentX.putdata(commentBuffer, cmdList);
...
void DrawMolItem::draw_dynamic_bonds(float *framepos, float brad, int bres, float maxdist) {
  ...
  sprintf(commentBuffer,"Mol[%d] Rep[%d] DynamicBonds", mol->id(), repNumber);
  cmdCommentX.putdata(commentBuffer, cmdList);
...
#if defined(VMDOPTIXRTRT)
      cylpoints.append3(coor1);
      cylpoints.append3(mid);
      cylradii.append(brad);
      cylcolors.append3(scene->color_value(atomColor->color[p->ind1]));
      cylpoints.append3(mid);
      cylpoints.append3(coor2);
      cylradii.append(brad);
      cylcolors.append3(scene->color_value(atomColor->color[p->ind2]));
#else
...
void DrawMolItem::draw_polyhedra(float *framepos, float maxdist) {
  ...
  sprintf(commentBuffer,"Mol[%d] Rep[%d] Polyhedra", mol->id(), repNumber);
  cmdCommentX.putdata(commentBuffer, cmdList);
...
void DrawMolItem::draw_structure(float *framepos, float brad, int bres, int linethickness) {
  sprintf (commentBuffer,"Mol[%d] Rep[%d] Cartoon", mol->id(), repNumber);
  cmdCommentX.putdata(commentBuffer, cmdList);
