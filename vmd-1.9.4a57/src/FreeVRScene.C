 ...
 *	$Revision: 1.43 $	$Date: 2020/02/26 15:51:10 $
 *
 ***************************************************************************/
/**
 *  \file FreeVRScene.C
...
//  draw_rwlock = vrLockCreate(vmdapp->freevrcontext); // allocate an RW Lock
  ...
  draw_barrier =(wkf_barrier_t *) malloc_from_FreeVR_memory(sizeof(wkf_barrier_t));
  ...
  if (getenv("VMDFREEVRDRAWPROCS") != NULL) {
    wkf_thread_barrier_init_proc_shared(draw_barrier, atoi(getenv("VMDFREEVRDRAWPROCS")) + 1);
  } else {
    wkf_thread_barrier_init_proc_shared(draw_barrier, vrContext->config->num_windows + 1);
  }
#endif
}
