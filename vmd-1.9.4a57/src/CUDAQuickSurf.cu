 ...
 *      $Revision: 1.97 $      $Date: 2021/12/21 05:30:12 $
 *
 ***************************************************************************/
/**
...
inline __device__ void convert_color(uchar4 & cu, float3 cf, float invisovalue) {
  ...
  cu = make_uchar4(fminf(cftmp.x * 255.0f, 255.0f),
                   fminf(cftmp.y * 255.0f, 255.0f),
                   fminf(cftmp.z * 255.0f, 255.0f),
                   255); 
#else
...
typedef struct {
  ...
  CUDAQuickSurf::VolTexFormat vtexformat;   ///< texture format associated with allocation
...
CUDAQuickSurf::CUDAQuickSurf() {
  ...
  gpuh->vtexformat = RGB3F; // large size allocs by default
...
int CUDAQuickSurf::calc_surf(long int natoms, const float *xyzr_f, 
                             ...
                             int colorperatom, VolTexFormat voltexformat,
...
printf("  ... bbe: %.2f %.2f %.2f\n", gorigin.x+bbox.x, gorigin.y+bbox.y, gorigin.z+bbox.z);
    ...
    if (colorperatom && (voltexformat == RGB4U)) {
