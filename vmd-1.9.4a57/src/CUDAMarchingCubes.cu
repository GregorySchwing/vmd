 ...
 *      $Revision: 1.44 $       $Date: 2022/03/18 20:39:06 $
 *
 ***************************************************************************/
/**
...
classifyVoxel(uint2 * RESTRICT voxelVerts, 
    ...
    // thread indexing depending on the GRIDIS3D template parameter
    if (GRIDIS3D) {
    ...
    numVerts.x = tex1Dfetch<unsigned int>(numVertsTexObj, int(cubeindex));
...
generateTriangleVerticesSMEM(float3 * RESTRICT pos, 
    ...
    unsigned int numVerts = tex1Dfetch<unsigned int>(numVertsTexObj, cubeindex);
    for(int i=0; i<numVerts; i+=3) {
        unsigned int index = numVertsScanned[voxel].x + i;

        float3 *vert[3];
        int edge;
        edge = tex1Dfetch<int>(triTexObj, (cubeindex*16) + i);
        vert[0] = &vertlist[(edge*NTHREADS)+threadIdx.x];

        edge = tex1Dfetch<int>(triTexObj, (cubeindex*16) + i + 1);
        vert[1] = &vertlist[(edge*NTHREADS)+threadIdx.x];

        edge = tex1Dfetch<int>(triTexObj, (cubeindex*16) + i + 2);
...
__global__ void 
// __launch_bounds__ ( KERNTHREADS, 1 )
generateTriangleNormals(const float3 * RESTRICT pos, NORMAL *norm, 
  ...
  n.x = tex3D<float>(volTexObj, p2.x, p2.y, p2.z) - tex3D<float>(volTexObj, p1.x, p1.y, p1.z);
  p1 = p + make_float3(0.0f, gridSizeInv.y, 0.0f);
  p2 = p - make_float3(0.0f, gridSizeInv.y, 0.0f);
  n.y = tex3D<float>(volTexObj, p2.x, p2.y, p2.z) - tex3D<float>(volTexObj, p1.x, p1.y, p1.z);
  p1 = p + make_float3(0.0f, 0.0f, gridSizeInv.z);
  p2 = p - make_float3(0.0f, 0.0f, gridSizeInv.z);
  n.z = tex3D<float>(volTexObj, p2.x, p2.y, p2.z) - tex3D<float>(volTexObj, p1.x, p1.y, p1.z);
...
void CUDAMarchingCubes::Cleanup() {
    if (d_triTable) {
      // destroy tex obj before freeing backing memory allocation
      cudaDestroyTextureObject(triTexObj);
      triTexObj = 0;
    ...
    if (d_numVertsTable) {
      // destroy tex obj before freeing backing memory allocation
      cudaDestroyTextureObject(numVertsTexObj);
      numVertsTexObj = 0;
    ...
    if (d_volumeArray) {
      // destroy tex obj before freeing backing memory allocation
      cudaDestroyTextureObject(volTexObj);
      volTexObj = 0;
...
//
// Methods that manage textures
//
void CUDAMarchingCubes::allocateTextures(int **d_triTab, unsigned int **d_numVertsTab) {
    //
    // new texture object implementation
    //

    // Build triTexObj 
    long tritexsz = 256*16*sizeof(int);
    cudaMalloc((void**) d_triTab, tritexsz);
    cudaMemcpy((void *)*d_triTab, (void *)triTable, tritexsz, cudaMemcpyHostToDevice);
    cudaResourceDesc resDescSigned;
    memset(&resDescSigned, 0, sizeof(resDescSigned));
    resDescSigned.resType = cudaResourceTypeLinear;
    resDescSigned.res.linear.desc.f = cudaChannelFormatKindSigned;
    resDescSigned.res.linear.desc.x = 32; // 32-bits in the X channel
    resDescSigned.res.linear.sizeInBytes = tritexsz;
    resDescSigned.res.linear.devPtr = *d_triTab;

    cudaTextureDesc texDesc;
    memset(&texDesc, 0, sizeof(texDesc));
    texDesc.readMode = cudaReadModeElementType;
  
    cudaCreateTextureObject(&triTexObj, &resDescSigned, &texDesc, NULL);


    // Build numVertsTexObj 
    long numvertsz = 256*sizeof(unsigned int);
    cudaMalloc((void**) d_numVertsTab, numvertsz);
    cudaMemcpy((void *)*d_numVertsTab, (void *)numVertsTable, numvertsz, cudaMemcpyHostToDevice);
    cudaResourceDesc resDescUnsigned;
    memset(&resDescUnsigned, 0, sizeof(resDescUnsigned));
    resDescUnsigned.resType = cudaResourceTypeLinear;
    resDescUnsigned.res.linear.desc.f = cudaChannelFormatKindUnsigned;
    resDescUnsigned.res.linear.desc.x = 32; // 32-bits in the X channel
    resDescUnsigned.res.linear.sizeInBytes = numvertsz;
    resDescUnsigned.res.linear.devPtr = *d_numVertsTab;

    cudaTextureDesc numVertsDesc;
    memset(&numVertsDesc, 0, sizeof(numVertsDesc));
    numVertsDesc.readMode = cudaReadModeElementType;
  
    cudaCreateTextureObject(&numVertsTexObj, &resDescUnsigned, &numVertsDesc, NULL);
}
...
void CUDAMarchingCubes::computeIsosurfaceVerts(float3* vertOut, unsigned int maxverts, dim3 & grid3) {
    ...
    if (cudacomputemajor >= 2) {
      ...
      if (useSubGrid) {
        classifyVoxel<1,1><<<grid3D, threads3D>>>(d_voxelVerts, numVertsTexObj,
                             d_volume, gridSize, numVoxels, voxelSize, 
                             subGridStart, subGridEnd, isoValue);
      } else {
        classifyVoxel<1,0><<<grid3D, threads3D>>>(d_voxelVerts, numVertsTexObj,
                             d_volume, gridSize, numVoxels, voxelSize, 
                             subGridStart, subGridEnd, isoValue);
      }
    } else {
      ...
      if (useSubGrid) {
        classifyVoxel<0,1><<<grid, threads>>>(d_voxelVerts, numVertsTexObj,
                             d_volume, gridSize, numVoxels, voxelSize,
                             subGridStart, subGridEnd, isoValue);
      } else {
        classifyVoxel<0,0><<<grid, threads>>>(d_voxelVerts, numVertsTexObj,
                             d_volume, gridSize, numVoxels, voxelSize,
                             subGridStart, subGridEnd, isoValue);
      }
    }
    ...
    generateTriangleVerticesSMEM<<<grid2, NTHREADS>>>(vertOut, 
                                   d_compVoxelArray, d_voxelVerts, 
                                   triTexObj, numVertsTexObj,
                                   d_volume, gridSize, voxelSize, 
...
void CUDAMarchingCubes::computeIsosurface(float3* vertOut, float3* normOut, float3* colOut, unsigned int maxverts) {
    ...
    if (this->useColor) {
      switch (texformat) {
        case RGB3F:
          generateTriangleColorNormal<float3, float3, float3><<<grid3, threads>>>(vertOut, colOut, normOut, (float3 *) d_colors, 
                  volTexObj, this->gridSize, gridSizeInv, bBoxInv, totalVerts);
          break;

        case RGB4U:
          generateTriangleColorNormal<float3, uchar4, float3><<<grid3, threads>>>(vertOut, colOut, normOut, (uchar4 *) d_colors, 
                  volTexObj, this->gridSize, gridSizeInv, bBoxInv, totalVerts);
          break;
      }
    } else {
      generateTriangleNormals<float3><<<grid3, threads>>>(vertOut, normOut, 
              volTexObj, gridSizeInv, bBoxInv, totalVerts);
    }
...
void CUDAMarchingCubes::computeIsosurface(float3* vertOut, float3* normOut, uchar4* colOut, unsigned int maxverts) {
    ...
    if (this->useColor) {
      switch (texformat) {
        case RGB3F:
          generateTriangleColorNormal<uchar4, float3, float3><<<grid3, threads>>>(vertOut, colOut, normOut, (float3 *) d_colors, 
                  volTexObj, this->gridSize, gridSizeInv, bBoxInv, totalVerts);
          break;

        case RGB4U:
          generateTriangleColorNormal<uchar4, uchar4, float3><<<grid3, threads>>>(vertOut, colOut, normOut, (uchar4 *) d_colors, 
                  volTexObj, this->gridSize, gridSizeInv, bBoxInv, totalVerts);
          break;
      }
    } else {
...
void CUDAMarchingCubes::computeIsosurface(float3* vertOut, char3* normOut, uchar4* colOut, unsigned int maxverts) {
    ...
    if (this->useColor) {
      switch (texformat) {
        case RGB3F:
          generateTriangleColorNormal<uchar4, float3, char3><<<grid3, threads>>>(vertOut, colOut, normOut, (float3 *) d_colors, 
                  volTexObj, this->gridSize, gridSizeInv, bBoxInv, totalVerts);
          break;

        case RGB4U:
          generateTriangleColorNormal<uchar4, uchar4, char3><<<grid3, threads>>>(vertOut, colOut, normOut, (uchar4 *) d_colors, 
                  volTexObj, this->gridSize, gridSizeInv, bBoxInv, totalVerts);
          break;
      }
    } else {
      generateTriangleNormals<char3><<<grid3, threads>>>(vertOut, normOut, 
              volTexObj, gridSizeInv, bBoxInv, totalVerts);
    }
...
bool CUDAMarchingCubes::SetVolumeData(float *volume, void *colors, 
  ...
  //
  // copy data to 3D array and volumetric texture volTexObj
  // 
  ...
  cudaResourceDesc resDescVol;
  memset(&resDescVol, 0, sizeof(resDescVol));
  resDescVol.resType = cudaResourceTypeArray;
  resDescVol.res.array.array = d_volumeArray;

  cudaTextureDesc volDesc;
  memset(&volDesc, 0, sizeof(volDesc));
  volDesc.normalizedCoords = 1;
  volDesc.filterMode = cudaFilterModeLinear;
  volDesc.addressMode[0] = cudaAddressModeClamp;
  volDesc.addressMode[1] = cudaAddressModeClamp;
  volDesc.addressMode[2] = cudaAddressModeClamp;
  volDesc.readMode = cudaReadModeElementType;

  cudaCreateTextureObject(&volTexObj, &resDescVol, &volDesc, NULL);
