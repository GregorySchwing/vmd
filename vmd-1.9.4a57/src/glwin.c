 ...
 *
 * $Id: glwin.c,v 1.35 2021/12/21 23:23:42 johns Exp $
 */
...
#include <GL/gl.h>
/* 3DxWare driver */
#if defined(VMDSPACEWARE) && (defined(WIN32) || defined(_WIN64))
#define OS_WIN32 1
...
#if defined(USEOPENGL) && !defined(USEEGL) && !(defined(WIN32) || defined(_WIN64)) && !defined(_MSC_VER)
...
LRESULT WINAPI myWindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  switch (msg) {
    ...
    case WM_KEYDOWN:
      ...
      if (handle->evkey != 0) {
        /* Windows platforms return a raw character (upper case)  */
        /* without applying shift-state modifiers, so we manually */
        /* query shift key modifier state and apply the modifier  */
        /* ourselves w/ toupper()/tolower().                      */
        int shiftstate = ((handle->MouseFlags & MK_SHIFT) != 0);
        handle->evkey = (shiftstate) ? toupper(handle->evkey) : tolower(handle->evkey);
      } else {
        ...
        unsigned int keysym = (unsigned int) wParam;
...
void glwin_draw_sphere_tex(float rad, int res, float txlatstart, float txlatend) {
  ...
  /* longitudinal "slices" */
  float ang_twopi_res = 6.28318530718f * res_1;
  for (i=0; i<res; i++) {
  ...
  /* latitude "stacks" */
  float ang_pi_res = 3.14159265359f * res_1;
  for (i=0; i<=res; i++) {
...
void glwin_spheremap_draw_tex(void *voidhandle, int stereomode, 
  ...
  a = wxs / ((float) wys); /* window aspect ratio */
  t = n * tanf(fov * 3.14159265359f / (180.0f*2.0f)); /* top */
  b = -t;        /* bottom */
  r = a * t;     /* right */
  l = -r;        /* left */
