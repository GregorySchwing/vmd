 ...
 *      $Revision: 1.46 $      $Date: 2020/11/04 20:57:00 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
int tcl_get_vecarray(const char *s, int &num, float *&val, Tcl_Interp *interp) {
  const char **pos;
  int cnt;
  if (Tcl_SplitList(interp, s, &cnt, &pos) != TCL_OK) {
    Tcl_SetResult(interp, (char *) "need numeric values", TCL_STATIC);
    return TCL_ERROR;
  }
  ...
  val = new float[num];
  for (int i=0; i<cnt; i++) { 
    if (tcl_get_vector(pos[i], val+i*3, interp) != TCL_OK) { 
      num = 0; 
      delete [] val;
      val = NULL;
      ckfree((char *) pos); // free of tcl data
...
int tcl_get_array(const char *s, int &num, float *&val, Tcl_Interp *interp) {
  ...
  for (int i=0; i<num; i++) { 
    double a;
    if (Tcl_GetDouble(interp, pos[i], &a) != TCL_OK) {
      num = 0; 
      delete [] val;
      val = NULL;
      ckfree((char *) pos); // free of tcl data
...
int tcl_get_intarray(const char *s, int &num, int *&val, Tcl_Interp *interp) {
  ...
  for (int i=0; i<num; i++) { 
    int a;
    if (Tcl_GetInt(interp, pos[i], &a) != TCL_OK) {
      num = 0; 
      delete [] val;
      val = NULL;
      ckfree((char *) pos); // free of tcl data
