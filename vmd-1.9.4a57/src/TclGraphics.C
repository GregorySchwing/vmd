 ...
 *	$Revision: 1.60 $	$Date: 2020/11/30 07:09:12 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
static int tcl_graphics_spheretube(MoleculeGraphics *gmol, int argc, const char *argv[], Tcl_Interp *interp) {
  // have a vector and some text
  AT_LEAST(2, "spheretube");
  float *coords = NULL; 
  float *radii = NULL;
  float radius = 0.0f;
  float *rgb3fv = NULL;
  int *colorids = NULL;
  int numcoords = 0, numcolors = 0, numcolorids = 0;
  int radset = 0, numradii = 0;
  int drawtubes = 0;
  int res = 8;
  
  if (tcl_get_vecarray(argv[0], numcoords, coords, interp) != TCL_OK) {
    delete [] coords;
    return TCL_ERROR;
  }

  // advance to process remaining command arguments
  argc -= 1;
  argv += 1;

  while (argc) {
    if (!strcmp(argv[0], "colors")) {
      if (tcl_get_vecarray(argv[1], numcolors, rgb3fv, interp) != TCL_OK) {
        delete [] coords;
        delete [] rgb3fv;
        delete [] colorids; 
        delete [] radii;
    ...
    if (!strcmp(argv[0], "colorids")) {
      if (tcl_get_intarray(argv[1], numcolorids, colorids, interp) != TCL_OK) {
        delete [] coords;
        delete [] rgb3fv;
        delete [] colorids; 
        delete [] radii;
      ...
      rgb3fv = new float[numcolorids * 3];
      for (int idx=0; idx<numcolorids; idx++) {
        int id = colorids[idx];
        // is it valid?
        if (id <0 && id >= MAXCOLORS) {
          char tmpstring[64];
          sprintf(tmpstring, "graphics spheretubes: color index value '%d' out of range", id);
          Tcl_SetResult(interp, tmpstring, TCL_VOLATILE);
          return TCL_ERROR;
        }
      }
    ...
    if (!strcmp(argv[0], "radii")) {
      if (tcl_get_array(argv[1], numradii, radii, interp) != TCL_OK) {
        delete [] coords;
        delete [] rgb3fv;
        delete [] colorids; 
        delete [] radii;
      ...
      radset = 1; // we got some radius information
      radius = float(radii[0]); // must set to first radius in case of 1 radii
    ...
    if (!strcmp(argv[0], "radius")) {
      double trad = 0.0;
      if (Tcl_GetDouble(interp, argv[1], &trad) != TCL_OK) {
        delete [] coords;
        delete [] rgb3fv;
        delete [] colorids; 
        delete [] radii;
    ...
    if (!strcmp(argv[0], "drawtubes")) {
      if (Tcl_GetInt(interp, argv[1], &drawtubes) != TCL_OK) {
        delete [] coords;
        delete [] rgb3fv;
        delete [] colorids; 
        delete [] radii;
    ...
    if (!strcmp(argv[0], "resolution")) {
      if (Tcl_GetInt(interp, argv[1], &res) != TCL_OK) {
        delete [] coords;
        delete [] rgb3fv;
        delete [] colorids; 
        delete [] radii;
    ...
    // reaching here is an error
    Tcl_AppendResult(interp, "graphics: unknown option for spheretube: ",
                     argv[0], NULL);
...
#if 0
  printf("***spheretube: coords %d radii %d drawtubes: %d res %d\n", 
         numcoords/3, numradii, drawtubes, res);
#endif

  if ((numcoords % 3)) {
    Tcl_SetResult(interp, (char *) "graphics: illegal spheretube coordinate vector length", TCL_STATIC);
    printf("total coordinate components not divible by 3!\n");
    delete [] coords;
    delete [] rgb3fv; 
    delete [] colorids; 
    delete [] radii;
  ...
  if ((numradii > 1) && (numradii != (numcoords/3))) {
    Tcl_SetResult(interp, (char *) "graphics: spheretube requires matching count of coords and radii", TCL_STATIC);
    delete [] coords;
    delete [] rgb3fv; 
    delete [] colorids; 
    delete [] radii;
  ...
  if ((numcolors > 0 || numcolorids > 0) && 
      (((numcolors != 0) && (numcolors != numcoords)) || 
       ((numcolorids != 0) && ((3*numcolorids) != numcoords)))) {
    Tcl_SetResult(interp, (char *) "graphics: spheretube requires matching count of coords and colors", TCL_STATIC);
    delete [] coords;
    delete [] rgb3fv; 
    delete [] colorids; 
    delete [] radii;
  ...
  if ((!radset)) {
    Tcl_SetResult(interp, (char *) "graphics: spheretube requires either radius or radii parameter be set", TCL_STATIC);
    delete [] coords;
    delete [] rgb3fv; 
    delete [] colorids; 
    delete [] radii;
  ...
  int colorcnt = 0;
  if (numcolors > 0)
    colorcnt = numcolors / 3;
  if (numcolorids > 0)
    colorcnt = numcolorids;

  char tmpstring[64];
  int gid = gmol->add_spheretube(numcoords/3, coords, numradii, 
                                 (numradii > 1) ? radii : &radius,
                                 colorcnt, rgb3fv, colorids, 
                                 drawtubes, res);
  sprintf(tmpstring, "%d", gid);
  Tcl_SetResult(interp, tmpstring, TCL_VOLATILE);
  return TCL_OK;
}
                        ...
                        Tcl_Interp *interp)
{
  ...
  if (!strcmp(argv[0], "spheretube")) {
    return tcl_graphics_spheretube(gmol, argc-1, argv+1, interp);
  }
