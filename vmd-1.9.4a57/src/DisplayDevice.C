 ...
 *	$Revision: 1.151 $	$Date: 2021/03/22 05:42:25 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
DisplayDevice& DisplayDevice::operator=( DisplayDevice &display) {
  ...
  // Clear and copy just the top item onto the stack.
  transMat.clear();
...
#if 1
    minX = float((int) ((float) (fminX + (fminX >= 0.0f ?  0.5f : -0.5f))));
    maxX = float((int) ((float) (fmaxX + (fmaxX >= 0.0f ?  0.5f : -0.5f))));
    minY = float((int) ((float) (fminY + (fminY >= 0.0f ?  0.5f : -0.5f))));
    maxY = float((int) ((float) (fmaxY + (fmaxY >= 0.0f ?  0.5f : -0.5f))));
#else
