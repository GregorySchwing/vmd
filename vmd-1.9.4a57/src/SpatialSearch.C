 ...
 *	$Revision: 1.26 $	$Date: 2020/07/08 04:23:47 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
GridSearchPair *vmd_gridsearch1(const float *pos,int natoms, const int *on, 
  ...
  if (isnan(volume)) {
    msgErr << "vmd_gridsearch1: bounding volume yielded NaN" << sendmsg;
    return NULL;
  }
...
GridSearchPair *vmd_gridsearch2(const float *pos,int natoms, 
  ...
  volume = fabsf((sidelen[0] + 2.0f) * (sidelen[1] + 2.0f) * (sidelen[2] + 2.0f));
  if (isnan(volume)) {
    msgErr << "vmd_gridsearch2: bounding volume yielded NaN" << sendmsg;
    return NULL;
  }
...
GridSearchPair *vmd_gridsearch3(const float *posA, int natomsA, const int *A, 
  ...
  volume = fabsf((sidelen[0] + 2.0f) * (sidelen[1] + 2.0f) * (sidelen[2] + 2.0f));
  if (isnan(volume)) {
    msgErr << "vmd_gridsearch3: bounding volume yielded NaN" << sendmsg;
    return NULL;
  }
