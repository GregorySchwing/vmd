 ...
 *      $Revision: 1.58 $       $Date: 2022/01/20 18:45:59 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
static int vmdinfo_tcl(ClientData, Tcl_Interp *interp,
  ...
  if (argc == 2) {
    ...
    // return the C++ compiler language standard/level used
    if (!strcmp(argv[1], "compilers")) { 
      const char *ccversion = c_compiler_std();

#if   (__cplusplus >= 202002L)
      const char *cxxversion = "C++ 2020";
#elif (__cplusplus >= 201703L)
      const char *cxxversion = "C++ 2017";
#elif (__cplusplus >= 201402L)
      const char *cxxversion = "C++ 2014";
#elif (__cplusplus >= 201103L)
      const char *cxxversion = "C++ 2011";
#elif (__cplusplus >= 199711L)
      const char *cxxversion = "C++ 2003";
#else 
      const char *cxxversion = "C++ 1998";
#endif

      Tcl_Obj *tcl_result = Tcl_NewListObj(0, NULL);
      Tcl_ListObjAppendElement(interp, tcl_result, Tcl_NewStringObj(ccversion, strlen(ccversion)));
      Tcl_ListObjAppendElement(interp, tcl_result, Tcl_NewStringObj(cxxversion, strlen(cxxversion)));
