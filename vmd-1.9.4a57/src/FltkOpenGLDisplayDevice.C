 ...
 *	$Revision: 1.77 $	$Date: 2020/04/23 07:07:42 $
 *
 ***************************************************************************/
/**
 * \file FltkOpenGLDisplayDevice.C
 * \brief OpenGLRenderer subclass managing FLTK window and input state,
 *        OpenGL GLX context, and framebuffer management.
 *
...
// FLTK High-DPI display support:
//   FLTK versions 1.3.5 and later incorporate Apple-specific extensions
//   for retina display support.  
//
//   On Apple MacOS when High-DPI Retina displays are enabled, 
//   FLTK apparently uses a hard-coded 2:1 pixel to drawing unit scale factor 
//
//   High-DPI support on Win32, Win64, and X-Windows require further
//   special handling available only in FLTK 1.4.x, which was redesigned with
//   a new per-screen scale factor:
//     https://www.fltk.org/doc-1.4/drawing.html
//
//   On Windows, X-Windows, the window-specific scale factor is:
//     int nscreen = windows->screen_num();
//     float s = Fl::screen_scale(nscreen);
//
//   The active scale factor can be overridden with the use of the
//   FLTK_SCALING_FACTOR environment variable, and FLTK windows
//   will respond to (cmd | ctrl)/+/-/0 (like a web browser) to allow
//   interactive adjustment of scaling factor.
//   FLTK 1.4.x adds Fl_Image::scale() to impact image drawing.
//
// Older discussions:
//   https://www.oyranos.org/2016/01/high-dpi-with-fltk/index.html
//   https://groups.google.com/forum/#!topic/fltkgeneral/qS-A50x6owk
//
#if defined(__APPLE__)
#if (FL_MAJOR_VERSION >= 1) && (((FL_MINOR_VERSION >= 3) && (FL_PATCH_VERSION >= 5)) || (FL_MINOR_VERSION >= 4))
#define VMDFLTKUSEHIGHDPIRETINA 1
#endif
#endif
...
#if defined(VMDFLTKUSEHIGHDPIRETINA)
  if (getenv("VMDNOHIGHDPI") != NULL) {
    msgWarn << "User override: High-DPI OpenGL display support disabled." << sendmsg;
  } else {
    // Enable High-DPI support on Apple Retina displays when supported by
    // the underlying FLTK version
    // Global FLTK function to activate support for high-DPI Apple Retina
    // displays.  This must be called before _any_ Fl_Gl_Window is shown.
    // When active, the pixel_w() and pixel_h() methods must be used instead
    // of w() and h(), e.g., glViewport(0, 0, pixel_w(), pixel_h()).
    Fl::use_high_res_GL(1);
    msgInfo << "High-DPI OpenGL display support enabled." << sendmsg;
  }
#endif
...
int FltkOpenGLDisplayDevice::x(void) {
  ...
  // Enable High-DPI support on Apple Retina displays when supported by
  // the underlying FLTK version
  // When FLTK high-DPI support is enabled, it is necessary to multiply
  // values returned by event_x() by the result of 
  // Fl_Gl_Window::pixels_per_unit() to get correct GL-window pixel 
  // coordinates.
  // XXX Docs bug: FLTK docs don't specify how event_x_root() is affected, but
  //     it appears to follow the same behavior as event_x().
...
int FltkOpenGLDisplayDevice::y(void) {
  ...
  // the underlying FLTK version
  // When FLTK high-DPI support is enabled, it is necessary to multiply
  // values returned by event_y() by the result of 
  // Fl_Gl_Window::pixels_per_unit() to get correct GL-window pixel 
  // coordinates.
  // XXX Docs bug: FLTK docs don't specify how event_y_root() is affected, but
  //     it appears to follow the same behavior as event_y().
...
void FltkOpenGLDisplayDevice::reshape(void) {
  ...
  // Enable High-DPI support on Apple Retina displays when supported by
  // the underlying FLTK version
  xSize = window->w() * window->pixels_per_unit();
  ySize = window->h() * window->pixels_per_unit();
...
#if defined(VMDFLTKUSEHIGHDPIRETINA)
  // when running in High-DPI / retina mode, the user specifies VMD image 
  // resolutions in real rendered pixels, not pseudo pixels, so we have to 
  // pre-divide out the scale factor since FLTK will multiply it back in.
  window->size(w / window->pixels_per_unit(), h / window->pixels_per_unit());
#else
  window->size(w, h);
#endif
