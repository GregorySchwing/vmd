...
* RCS INFORMATION:
*
*      $RCSfile: OptiXDisplayDevice.C,v $
*      $Author: johns $      $Locker:  $               $State: Exp $
*      $Revision: 1.86 $         $Date: 2021/12/21 06:02:25 $
*
***************************************************************************/
/**
 *  \file OptiXDisplayDevice.C
...
OptiXDisplayDevice::OptiXDisplayDevice(VMDApp *app, int interactive) : FileRenderer((interactive) ? 
...
/// wrap OptiXRenderer to eliminate overly broad OptiX header inclusion
unsigned int OptiXDisplayDevice::device_count(void) {
  return OptiXRenderer::device_count();
}
...
void OptiXDisplayDevice::add_material(void) {
  ort->add_material(materialIndex,
                    ...
                    roundf(mat_transmode)); // XXX allow shader hacking
//                    mat_transmode > 0.5f);
}
...
void OptiXDisplayDevice::send_sphere_buffer() {
  if (sphere_vert_buffer.num() > 0) {
    // send the spheres...
    ort->sphere_array_color(*sphere_xform, sphere_radius_scalefactor,
                            sphere_vert_buffer.num()/3,
                            &sphere_vert_buffer[0],
                            &sphere_radii_buffer[0],
                            &sphere_color_buffer[0],
                            sphere_matindex);
...
void OptiXDisplayDevice::sphere(float *xyzr) {
#if 0
  add_material();

  // Submit all geometry in one buffer
  ort->sphere_array_color(transMat.top(), scale_factor(), 1,
                          xyzr, xyzr+3, &matData[colorIndex][0], materialIndex);
#else
  // if we have a change in transformation matrix, color, or material state,
  // we have to emit all accumulated cylinders to OptiX and begin a new batch
  if (
      ...
      // OptiX buffers, to better facilitate NVLink-based distributed memory 
      // ray tracing across multiple GPUs.
      sphere_vert_buffer.num() >= 15000000 || 
#endif
      (sphere_xform != NULL && ((sphere_matindex != materialIndex) || (memcmp(sphere_xform->mat, transMat.top().mat, sizeof(sphere_xform->mat)))))) {
    send_sphere_buffer(); // render the accumulated sphere buffer...
  }

  // record all transformation/material/color state on first sphere call
  if (sphere_xform == NULL) {
    // record material, color, and transformation state
    sphere_matindex = materialIndex;
    sphere_xform = new Matrix4(transMat.top());
    sphere_radius_scalefactor = scale_factor();
    add_material(); // cause OptiX to cache the current material
  }

  // record vertex data
  sphere_vert_buffer.append3(xyzr);
  sphere_radii_buffer.append(xyzr[3]);
  sphere_color_buffer.append3(&matData[colorIndex][0]);
#endif
}
...
void OptiXDisplayDevice::trimesh_n3b_v3f(signed char *n, float *v, 
                                         int numfacets) {
  add_material();
