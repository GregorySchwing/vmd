...
/**
 *  \file Displayable.C
 *  \brief Base class for all objects which are drawn in the DisplayDevice.
 * 
 ...
 *     means it gives the display command list pointer to the Scene, 
 *     which then uses that to draw the object.
 *     Before drawing, the Scene calls the 'prepare' routine for each 
 *     Displayable.  When deleted, the Displayable 'unregisters' with 
 *     all Scene objs it has previously registered with.
 *     If a Scene is deleted, it unregisters the Displayables in 
 *     it's possession.
 * 
 *  4. A Displayable is either a 'parent' one or a 'child' one.  The
 *     difference is that parent Displayables register with the scene, 
 *     and have only one transformation; children do NOT register with 
 *     the scene, and have not only their main transformation 
 *     (which is the same as the parent) but also a second one 
 *     which multiplies the first.  Note that children can have
 *     children of their own, but are not 'parents', 
 *     i.e. child Displayables do not register with a scene even if 
 ...
 *     specified instead of the Scene.  As part of the creation, the 
 *     child will be added to the parent's list via the 'add child' routine.
 */
