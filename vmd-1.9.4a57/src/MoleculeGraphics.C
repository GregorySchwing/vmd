 ...
 *	$Revision: 1.89 $	$Date: 2021/12/21 05:59:15 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
void MoleculeGraphics::create_cmdlist(void) {
  ...
  while (sidx<num) {
    ...
    switch (shape->shape) {
      ...
      case PICKPOINT: 
        pickpointcoords.append3(shape->data);
        pickpointindices.append((int)shape->data[3]);
        break;
      ...
      case TEXT: {
        append(DMATERIALOFF);
        DispCmdText text;
        text.putdata(shape->data, shapetext[(int)shape->data[5]], shape->data[4], shape->data[3], 0, 0, cmdList);
        append(DMATERIALON);
        break;
      }
      ...
      case SPHERETUBE: {
        ...
        const int numcoords   = int(shape->data[0]);
        const int numradii    = int(shape->data[1]);
        const float defradius = shape->data[2];
//        const int numcolors   = shape->data[3]);
        const int drawtubes   = int(shape->data[4]);
        const int res         = int(shape->data[5]);

#if 0
printf("***** spheretube draw: coords %d rads %d defrad %f cols %d tubes %d res %d\n",
       numcoords, numradii, defradius, numcolors, drawtubes, res);
#endif

        const int coordsz = numcoords * 3; 
        const int colorsz = numcoords * 3; // we MUST provide per-sphere colors
        const float *xyz3fv = (float *) shape->extradata;
        const float *rgb3fv = xyz3fv + coordsz;
        const float *radii1fv = xyz3fv + coordsz + colorsz;

        // use either a constant color for all spheres, or use a 
        // color-per-sphere 
        float *colors = NULL;
        float *tmpradii = NULL; 
        const float *useradii = NULL;
        if ((numradii > 1) && (numradii == numcoords)) {
          useradii = radii1fv;
// printf("** using radii array radii[0] = %g\n", useradii[0]);
        } else {
// printf("** defradius: %f\n", defradius);
          tmpradii = new float[numradii];
          for (int i=0; i<numcoords; i++) {
            tmpradii[i] = defradius;
          }
          useradii = tmpradii;
// printf("** using defradius radii[0] = %g\n", useradii[0]);
        }

        DispCmdSphereArray cmdSphereArray;
        cmdSphereArray.putdata(xyz3fv,
                               useradii,
                               rgb3fv,
                               numcoords,
                               res,
        ...
        // optionally draw interconnecting tubes
        if (drawtubes) {
          // if tubes have varying radii, we must draw cones
          int i,j;
          for (i=0,j=0; i<(numcoords-1); i++,j+=3) {
// XXX we don't handle color-per sphere yet
            cone.putdata(xyz3fv+j, xyz3fv+j+3, 
                         useradii[i], useradii[i+1], res, cmdList);
          }
        }
...
int MoleculeGraphics::add_spheretube(const int numcoords, const float *xyz3fv, 
                                     const int numradii, const float *radii1fv,
                                     const int numcolors,
                                     const float *rgb3fv, const int *colorids,
                                     int drawtubes, int res) {
  ShapeClass s(SPHERETUBE, 6, next_id); 
  float *data = s.data;
  data[0] = float(numcoords);
  data[1] = float(numradii);
  data[2] = radii1fv[0];  // if numradii == 1, then this is the global radius
  data[3] = float(numcolors);    // if numcolors == 0, we pack a 1-color array
  data[4] = float(drawtubes);
  data[5] = float(res);

  // store bulk coordinates in an attached memory buffer
  const int coordsz = numcoords * 3;
  const int colorsz = numcoords * 3; // we MUST provide per-sphere colors
  float *tmp = (float *) malloc((coordsz + colorsz + numradii) * sizeof(float));
  memcpy(tmp, xyz3fv, coordsz * sizeof(float));

  // we must provide full color-per-sphere data presently
  if (numcolors < numcoords) {
    // use last draw color state, copied to buffer of elements
    const float *tc = scene->color_value(colorID);
    for (int i=0; i<colorsz; i+=3) {
      vec_copy(tmp+coordsz + i, tc);
    }
  } else {
    if (rgb3fv != NULL) {
      memcpy(tmp+coordsz, rgb3fv, colorsz * sizeof(float));
    } if (colorids != NULL) {
      for (int i=0; i<numcolors; i++) {
        int c = colorids[i];
        c = clamp_int(c, 0, MAXCOLORS-1);
        vec_copy(tmp+coordsz + i*3, scene->color_value(c));
      }
    }
  }

  memcpy(tmp+coordsz+colorsz, radii1fv, numradii * sizeof(float));
  s.extradata = tmp;
...
int MoleculeGraphics::use_color(int index) {
  colorID = index; // record most recently set color index
...
const char *MoleculeGraphics::info_id(int id) {
  ...
  if (graphics_info_xl != NULL)
    delete graphics_info_xl;
  ...
  case SPHERETUBE: {
    ...
    const int numradii    = int(shape->data[1]);
    const float defradius = shape->data[2];
//    const int numcolors   = int(shape->data[3]);
    const int drawtubes   = int(shape->data[4]);
    const int res         = int(shape->data[5]);
    const int coordsz     = numcoords * 3; 
    const int colorsz     = numcoords * 3; // we MUST provide per-sphere colors
    ...
    graphics_info_xl = new JString("spheretube {");
    for (i=0; i<coordsz; i+=3) {
      sprintf(graphics_info, "{%g %g %g} ", xyz3fv[i], xyz3fv[i+1], xyz3fv[i+2]);
      *graphics_info_xl += (const char *) graphics_info;
    }    
    *graphics_info_xl += "} ";

    if (numradii > 1) {
       *graphics_info_xl += "radii {";
       for (i=0; i<numcoords; i++) {
         sprintf(graphics_info, "%g ", radii1fv[i]);
         *graphics_info_xl += (const char *) graphics_info;
       }    
       *graphics_info_xl += "} ";
    } else {
       sprintf(graphics_info, "radius %g ", defradius);
       *graphics_info_xl += "} ";
    }

    // XXX incorrect logging at present
    // XXX right now this will result in colorID arrays getting converted
    // into RGB values incorrectly, we'll need to store them
    *graphics_info_xl += "colors {";
    for (i=0; i<colorsz; i+=3) {
      sprintf(graphics_info, "{%g %g %g} ", rgb3fv[i], rgb3fv[i+1], rgb3fv[i+2]);
      *graphics_info_xl += (const char *) graphics_info;
    }    
    *graphics_info_xl += "} ";

    if (drawtubes) {
      *graphics_info_xl += "drawtubes 1 ";
    }

    sprintf(graphics_info, "resolution %d ", res);
    *graphics_info_xl += (const char *) graphics_info;
    return ((const char *) *graphics_info_xl);
  }
  case MATERIALS: {
...
void MoleculeGraphics::find_bounds(void) {
  ...
  for (int i=0; i<num; i++) {
    ...
    case SPHERETUBE: {
      int numcoords = int(shape->data[0]);
      float lmin[3], lmax[3];
      const float *coords = (const float *) shape->extradata;
      minmax_3fv_aligned(coords, numcoords, lmin, lmax);
      minx = lmin[0];
      miny = lmin[1];
      minz = lmin[2];
      maxx = lmax[0];
