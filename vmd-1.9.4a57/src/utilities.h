 ...
 *	$Revision: 1.121 $	$Date: 2022/03/25 05:16:49 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
// forward declarations for WKFThreads typedefs
...
// macro workaround for MacOS X failing to expose C99 math routines in
// their standard math headers.  This is probably unnecessary with 
// later revs of MacOS X, but is necessary for MacOS X 10.10 and earlier.
#ifdef __APPLE__
...
extern int analyze_selection_aligned_dispatch(wkf_cpu_caps_t *cpucaps,
                                              int n, const int *on,
                                              int *firstsel, int *lastsel, 
                                              int *selected);

/// find min/max/mean values for an array of floats
extern void minmaxmean_1fv_aligned(const float *f, ptrdiff_t n, 
                                   float *fmin, float *fmax, float *fmean);

/// find min/max values for an array of floats
extern void minmax_1fv_aligned(const float *f, ptrdiff_t n, float *min, float *max);

// Compute min/max values for a 16-byte-aligned array of floats
// input value n3 is the number of 3-element vectors to process
extern void minmax_3fv_aligned(const float *f, const ptrdiff_t n3, float *fmin, float *fmax);
...
inline void vectrans(float *npoint, float *mat, double *vec){
  npoint[0]=float(vec[0]*mat[0]+vec[1]*mat[4]+vec[2]*mat[8]);
  npoint[1]=float(vec[0]*mat[1]+vec[1]*mat[5]+vec[2]*mat[9]);
  npoint[2]=float(vec[0]*mat[2]+vec[1]*mat[6]+vec[2]*mat[10]);
}
...
int k_candidates(int,int,int,int,float,float*,float*); // generates K candidates, returns farthest away from all previous samples
float arcdistance(float,float,float,float); //arcdistance between 2 pts on unit sphere
void print_xyz(float*,int); //print poisson set to .xyz file for validation
float correction(int); //returns minimum distance given N samples
int poisson_sample_on_sphere(float*,int,int,int); //Poisson disk sampling procedure
// -- 


// Hardware-specific SIMD query routines available only via
// runtime CPU dispatch checks
#if defined(VMDCPUDISPATCH) && (defined(__ARM_ARCH_ISA_A64) || defined(__ARM_NEON))
int arm_sve_vecsize_32bits();
int arm_sve_vecsize_64bits();
#endif
