 ...
 *	$Revision: 1.170 $	$Date: 2020/07/23 03:27:52 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
int access_tcl_atomsel(ClientData my_data, Tcl_Interp *interp,
  ...
  case 4: { // get
    ...
    int *mapping;
    int num_mapping;
    if (split_tcl_atomsel_info(interp, atomSelParser, argv[2], 
                               &num_mapping, &mapping) != TCL_OK) {
  ...
  case 23: {   // writeXXX <name>
    ...
    spec.waitfor = FileSpec::WAIT_ALL; // wait for all frames to be written
