...
* RCS INFORMATION:
*
*      $RCSfile: OSPRay2Renderer.h,v $
*      $Author: johns $      $Locker:  $               $State: Exp $
*      $Revision: 1.12 $         $Date: 2021/12/13 07:54:00 $
*
***************************************************************************/
/**
 *  \file OSPRay2Renderer.h
...
#ifndef LIBOSPRAY2RENDERER
#define LIBOSPRAY2RENDERER
...
typedef struct {
  ...
  float *vertsrads;  // point a, rad a, point b, rad b
  float *colors; // rgba
...
private:
  ...
  RenderMode osp_rendermode;              ///< OSPRay 2.x renderer mode
  int context_created;                    ///< flag when context is valid
  OSPRenderer ospRenderer;                ///< OSPRay renderer
  OSPWorld ospWorld;                      ///< scene graph
  ResizeArray<OSPInstance> ospInstances;  ///< list of all instances in scene

  ResizeArray<OSPLight> ospLights;        ///< light objects
