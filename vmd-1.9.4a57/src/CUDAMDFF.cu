 ...
 *      $Revision: 1.84 $      $Date: 2021/12/21 06:59:27 $
 *
 ***************************************************************************/
/**
 * \file CUDAMDFF.cu
...
static int map_uniform_spacing(double xax, double yax, double zax,
  ...
  if ((xdelta == ydelta) && (ydelta == zdelta))
    return 1;
...
static int calc_density_bounds_overlapping_map(int verbose, float &gridspacing,
  ...
  float refxdelta = float(refmap->xaxis[0] / (refmap->xsize - 1));
  float refydelta = float(refmap->yaxis[1] / (refmap->ysize - 1));
  float refzdelta = float(refmap->zaxis[2] / (refmap->zsize - 1));
  if (verbose) {
  ...
  float fvoxorigin[3];
  fvoxorigin[0] = float((origin[0] - refmap->origin[0]) / refxdelta); 
  fvoxorigin[1] = float((origin[1] - refmap->origin[1]) / refydelta); 
  fvoxorigin[2] = float((origin[2] - refmap->origin[2]) / refzdelta); 
  ...
  refmaxcorner[0] = float(refmap->origin[0] + refmap->xaxis[0]);
  refmaxcorner[1] = float(refmap->origin[1] + refmap->yaxis[1]);
  refmaxcorner[2] = float(refmap->origin[2] + refmap->zaxis[2]);
  ...
  origin[0] = float(refmap->origin[0] + refoffset.x * refxdelta);
  origin[1] = float(refmap->origin[1] + refoffset.y * refydelta);
  origin[2] = float(refmap->origin[2] + refoffset.z * refzdelta);
...
int vmd_cuda_calc_density(const AtomSel *sel, MoleculeList *mlist, 
  ...
  if (refmap) {
    gridspacing = float(refmap->xaxis[0] / (refmap->xsize - 1));
    if (verbose) {
  ...
  int rc=0;
  if (!diffvol) {
    rc = vmd_cuda_gaussdensity_calc(sel->selected, acoriginoffset, acvolsz,
                                    xyzr, volmap, volsz, 
                                    maxrad, radscale, gridspacing, gausslim,
                                    refsz, refoffset, NULL, NULL,
                                    verbose);
    // fall back to CPU if GPU routines fail
    if (rc) {
      free(xyzr);
      delete [] volmap;
      return -1;
