...
* RCS INFORMATION:
*
*      $RCSfile: ANARIDisplayDevice.C,v $
*      $Author: johns $      $Locker:  $               $State: Exp $
*      $Revision: 1.3 $         $Date: 2021/05/14 22:13:07 $
*
***************************************************************************/
/**
 *  \file ANARIDisplayDevice.C
...
void ANARIDisplayDevice::ANARI_Global_Init(void) {
  ANARIRender::ANARI_Global_Init(); // call only ONCE
}

// Global ANARI shutdown routine -- call it only ONCE...
void ANARIDisplayDevice::ANARI_Global_Shutdown(void) {
  ANARIRender::ANARI_Global_Shutdown(); // call only ONCE
}


/// constructor ... initialize some variables
ANARIDisplayDevice::ANARIDisplayDevice(VMDApp *app, int interactive) : FileRenderer((interactive) ? 
               "ANARIInteractive" : "ANARIInternal", 
               (interactive) ? 
               "ANARI (interactive)" : "ANARI (internal, in-memory)",
...
/// destructor
ANARIDisplayDevice::~ANARIDisplayDevice(void) {
  delete ort;
  ort = NULL;
  wkf_timer_destroy(ort_timer);
}

void ANARIDisplayDevice::add_material(void) {
...
void ANARIDisplayDevice::reset_vars(void) {
//  reset_cylinder_buffer();
  reset_triangle_buffer();
}


void ANARIDisplayDevice::comment(const char *s) {
  ort->comment(s);
}
...
void ANARIDisplayDevice::send_cylinder_buffer() {
...
// draw a cylinder
void ANARIDisplayDevice::cylinder(float *a, float *b, float r, int filled) {
  // if we have a change in transformation matrix, color, or material state,
  // we have to emit all accumulated cylinders to ANARI and begin a new batch
  ...
  if (cylinder_xform == NULL) {
    ...
    add_material(); // cause ANARI to cache the current material
  }
...
#if 0
  // XXX Cylinder caps not yet supported in ANARI
  // Cylinder caps?
  if (filled) {
    float norm[3];
    norm[0] = b[0] - a[0];
...
// draw a sphere array
void ANARIDisplayDevice::sphere_array(int spnum, int spres, float *centers, 
...
//
void ANARIDisplayDevice::text(float *pos, float size, float thickness,
                              const char *str) {
  float textpos[3];
...
void ANARIDisplayDevice::send_triangle_buffer() {
...
// draw a triangle
void ANARIDisplayDevice::triangle(const float *a, const float *b, const float *c, const float *n1, const float *n2, const float *n3) {
  // if we have a change in transformation matrix, color, or material state,
  // we have to emit all accumulated triangles to ANARI and begin a new batch
...
// draw a tricolor
void ANARIDisplayDevice::tricolor(const float *a, const float *b, const float *c,
...
void ANARIDisplayDevice::trimesh_c4u_n3b_v3f(unsigned char *c, signed char *n,
...
void ANARIDisplayDevice::trimesh_c4u_n3f_v3f(unsigned char *c, float *n,
                                             float *v, int numfacets) {
  add_material();
  ort->trimesh_c4u_n3f_v3f(transMat.top(), c, n, v, numfacets, materialIndex);
}

void ANARIDisplayDevice::trimesh_c4n3v3(int numverts, float * cnv,
...
void ANARIDisplayDevice::trimesh_n3b_v3f(signed char *n, float *v, int numfacets) {
  add_material();
  ort->trimesh_n3b_v3f(transMat.top(), matData[colorIndex], n, v, numfacets,  materialIndex);
}

void ANARIDisplayDevice::trimesh_n3f_v3f(float *n, float *v, int numfacets) {
  add_material();
...
#if 0
void ANARIDisplayDevice::trimesh_n3fopt_v3f(float *n, float *v, int numfacets) {
  add_material();
...
void ANARIDisplayDevice::tristrip(int numverts, const float * cnv,
...
void ANARIDisplayDevice::write_lights() {
  int i;
  int lightcount = 0;

  // clear all existing lights before (re)appending the current lights,
  // otherwise if the ANARI context is reused, we will crash and burn.
  ...
  for (i=0; i<DISP_LIGHTS; i++) {
    if (advLightState[i].on) {
      ...
      if (advLightState[i].spoton) {
        printf("ANARIDisplayDevice) SpotLight not implemented yet ...\n");
      } else {
        apitexture tex;
...
void ANARIDisplayDevice::write_materials() {
  ort->set_bg_color(backColor);

  // Specify ANARI background sky sphere if background gradient
  // mode is enabled.
  if (backgroundmode == 1) {
    ...
    if (projection() == DisplayDevice::ORTHOGRAPHIC)
      ort->set_bg_mode(ANARIRender::RT_BACKGROUND_TEXTURE_SKY_ORTHO_PLANE);
    else
      ort->set_bg_mode(ANARIRender::RT_BACKGROUND_TEXTURE_SKY_SPHERE);
    ...
    ort->set_bg_mode(ANARIRender::RT_BACKGROUND_TEXTURE_SOLID);
  }
}
...
void ANARIDisplayDevice::write_header() {
  ...
  if (cueingEnabled) {
    switch (cueMode) {
      case CUE_LINEAR:
        ort->set_cue_mode(ANARIRender::RT_FOG_LINEAR, start, end, density);
        break;

      case CUE_EXP:
        ort->set_cue_mode(ANARIRender::RT_FOG_EXP, start, end, density);
        break;

      case CUE_EXP2:
        ort->set_cue_mode(ANARIRender::RT_FOG_EXP2, start, end, density);
        break;
    ...
    ort->set_cue_mode(ANARIRender::RT_FOG_NONE, start, end, density);
  }
}


void ANARIDisplayDevice::write_trailer(void){
//  send_cylinder_buffer(); // send any unsent accumulated cylinder buffer...
...
#if 0
  printf("ANARI: z: %f zDist: %f vSize %f\n", eyePos[2], zDist, vSize);
#endif
  switch (projection()) {
    case DisplayDevice::ORTHOGRAPHIC:
      ort->set_camera_projection(ANARIRender::RT_ORTHOGRAPHIC);
      ort->set_camera_zoom(0.5f / (1.0 / (vSize / 2.0)));
      break;

    case DisplayDevice::PERSPECTIVE:
    default:
      ort->set_camera_projection(ANARIRender::RT_PERSPECTIVE);
  ...
  if (getenv("VMDANARINODESTROYCONTEXT") == NULL) {
    ...
    // make a new ANARIRender object so we're ready for the next run...
    ort = new ANARIRender();
  } else {
  ...
  printf("ANARIDisplayDevice) Total rendering time: %.2f sec\n", wkf_timer_time(ort_timer));
}
