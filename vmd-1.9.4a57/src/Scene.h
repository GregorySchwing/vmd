 ...
 *	$Revision: 1.69 $	$Date: 2020/12/24 04:21:51 $
 *
 ***************************************************************************/
/**
 *  \file Scene.h
...
private:
  ...
  int scaleActive;              ///< active color scale index     
  float scaleMin, scaleMid, scaleMax;
  int scaleReverse;
  int scalePosterize;
  ResizeArray<ColorScale> colorScales;
  ResizeArray<const char *> colorScaleMenuNames;
...
public:
  ...
  void set_colorscale_params(float min, float mid, float max, 
                             int reverse, int posterize) {
    scaleMin = min; scaleMid = mid; scaleMax = max;
    scaleReverse = reverse;
    scalePosterize = posterize;
    create_colorscale();
  }
  void set_colorscale_method(int index) {
    if (scaleActive != index) {
      scaleActive = index;
      create_colorscale();
    }
  }
  ...
  /// color scale methods
  void colorscale_params(float *mid, float *min, float *max,
                         int *reverse, int *posterize) const {
    *mid = scaleMid; *min = scaleMin; *max = scaleMax; 
    *reverse = scaleReverse;
    *posterize = scalePosterize;
  }

  int colorscale_method() const { return scaleActive; }

  int num_colorscale_methods() const { return int(colorScales.num()); }
  const char *colorscale_method_name(int n) const {
    return colorScales[n].name;
  }
  const char *colorscale_method_menuname(int n) const {
    return colorScaleMenuNames[n];
  }
