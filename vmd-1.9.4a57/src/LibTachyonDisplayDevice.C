...
*      $Revision: 1.99 $        $Date: 2021/03/06 20:51:52 $
*
***************************************************************************/
/**
 *  \file LibTachyonDisplayDevice.C
 *  \brief FileRenderer subclass for the built-in Tachyon ray tracing engine.
 */
...
#if ((TACHYON_MAJOR_VERSION == 0) && (TACHYON_MINOR_VERSION == 99) && (TACHYON_PATCH_VERSION <= 1))
    rt_ambient_occlusion(rtscene, aosamples, skycol);
#else
    float ao_maxdist = RT_AO_MAXDIST_UNLIMITED;
 
    if (getenv("VMDTACHYONAOMAXDIST")) {
      float tmp = atof(getenv("VMDTACHYONAOMAXDIST"));
      printf("LibTachyonDisplayDevice) setting AO maxdist: %f\n", tmp);
      ao_maxdist = tmp;
    }

    rt_ambient_occlusion(rtscene, aosamples, ao_maxdist, skycol);
#endif
  } 
