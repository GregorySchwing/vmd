 ...
 *      $Revision: 1.34 $      $Date: 2020/10/15 17:44:27 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
//    if (!strcmp(opt, "--allframes")) {use_all_frames = true;}
  ...
  if (getenv("VMDNOCUDA") == NULL) {
    ...
    if(cuda_err != -1) {
      int newvolmolid = init_new_volume_molecule(app, synthvol, "sim_map");
  ...
  if (cuda_err == -1) {
   ...
   int newvolmolid = init_new_volume_molecule(app, volmap, "sim_map");
  Molecule *newvolmol = mlist->mol_from_id(newvolmolid);
  if (outputmap != NULL) {
    if (!write_file(app, newvolmol, 0, outputmap)){
      Tcl_AppendResult(interp, "\n no target volume correctly specified",NULL);
