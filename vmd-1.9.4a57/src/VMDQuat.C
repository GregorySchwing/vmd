 ...
 *      $Revision: 1.14 $      $Date: 2019/09/27 05:28:50 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
void Quat::rotate(const float *u, float angle) {
  Quat q;
  double halftheta = 0.5 * DEGTORAD(angle);
  double sinhalftheta, coshalftheta;
  sincos(halftheta, &sinhalftheta, &coshalftheta);
  q.qw = coshalftheta;
  q.qx = u[0]*sinhalftheta;
  q.qy = u[1]*sinhalftheta;
  q.qz = u[2]*sinhalftheta;
  mult(q);
}
...
void Quat::rotate(char axis, float angle) {
  ...
  q.qw = coshalftheta;
  switch(axis) {
    case 'x': q.qx = sinhalftheta; q.qy = q.qz = 0; break;
    case 'y': q.qy = sinhalftheta; q.qz = q.qx = 0; break;
    case 'z': q.qz = sinhalftheta; q.qx = q.qy = 0; break;
  }
  mult(q); 
}
