...
* RCS INFORMATION:
*
*      $RCSfile: OptiXRenderer.C,v $
*      $Author: johns $      $Locker:  $               $State: Exp $
*      $Revision: 1.391 $         $Date: 2021/12/21 06:44:17 $
*
***************************************************************************/
/**
 *  \file OptiXRenderer.C
 *  \brief VMD built-in Tachyon/OptiX ray tracing engine.
 * 
...
// low-level OptiX headers have to be included first 
// to prevent issues with compilation on Windows due to 
// header-internal min/max macro (re)definition, etc.
...
#include <cuda_runtime.h>   // needed for cudaGetDeviceCount() etc
#endif
...
#include "DispCmds.h"       // needed to process VMDDisplayList command tokens
#endif

// Enable the use of RT_GEOMETRY_BUILD_FLAG_RELEASE_BUFFERS on
// RTX-specific triangle mesh vertex/index buffers optimization
//
// XXX This is presently disabled due to observed performance 
//     losses compared to self-managed buffer deallocation when
//     rendering frames in a tight loop.
// #define VMDOPTIXRTXRELEASEBUFS 1
...
__forceinline__ int hwtri_test_calc_Ngeom(const float3 * __restrict__ vertices, 
                                          float3 &Ngeometric) {
...
static void hwtri_set_vertex_flags(RTcontext ctx, 
  ...
  if (lasterror != RT_SUCCESS) {
    printf("OptiXRenderer::hwtri_set_vertex_flags() error!\n");
  }
}
...
int OptiXPrintRayStats(RTbuffer raystats1_buffer, RTbuffer raystats2_buffer,
  ...
  printf("OptiXRenderer) ----------------------------------------\n");
  printf("OptiXRenderer) Image resolution: %lu x %lu \n", 
...
OptiXRenderer::OptiXRenderer(VMDApp *vmdapp) {
...
#if defined(ORT_USERTXAPIS)
  rtx_enabled = 1;             // RTX execution mode enabled
  ...
  if (getenv("VMDOPTIXCLIPVIEWSTART")) {
    clipview_start = float(atof(getenv("VMDOPTIXCLIPVIEWSTART")));
    msgInfo << "OptiXRenderer) Overriding default clipping start: " 
            << clipview_start << sendmsg;
  }
  if (getenv("VMDOPTIXCLIPVIEWEND")) {
    clipview_start = float(atof(getenv("VMDOPTIXCLIPVIEWEND")));
...
unsigned int OptiXRenderer::device_list(int **devlist, char ***devnames) {
  ...
  if (devcntresult != RT_SUCCESS) {
    ...
    if (devcntresult == RT_ERROR_OPTIX_NOT_LOADED) {
      ...
      if (cudadevcount > 0) {
        printf("OptiXRenderer) ERROR: Failed to load OptiX shared library.\n");
...
void OptiXRenderer::create_context() {
...
#if defined(ORT_USERTXAPIS)
  // Starting with OptiX 6.0.0, the OptiX runtime can select either an 
  // RTX execution strategy, or a classic non-RTX execution strategy
  // using a new global API flag that takes effect at context creation time.
  // RTX mode is only supported on Maxwell and later
  ...
  // The OptiX RTX execution strategy mode has to be set very early on,
...
void OptiXRenderer::setup_context(int w, int h) {
  ...
  if (getenv("VMDOPTIXMAXDEPTH")) {
    ...
    if (maxdepth > 0 && maxdepth <= 30) {
      ...
      scene_max_depth = maxdepth; // set context-wide property
      RTERR( rtVariableSet1ui(max_depth_v, scene_max_depth) );
    } else {
  ...
  if (getenv("VMDOPTIXMAXTRANS")) {
    int maxtrans = atoi(getenv("VMDOPTIXMAXTRANS"));
    if (maxtrans > 0 && maxtrans <= 30) {
      printf("OptiXRenderer) Setting maxtrans to %d...\n", maxtrans);
      scene_max_trans = maxtrans; // set context-wide property
      RTERR( rtVariableSet1ui(max_trans_v, scene_max_trans) );
    } else {
...
void OptiXRenderer::destroy_scene() {
  ...
  if (scene_created) {
    ...
    RTERR( rtAccelerationDestroy(trianglesacceleration) );
#endif
...
#if defined(ORT_USERTXAPIS)
    // OptiX RTX hardware-accelerated triangles API
    RTERR( rtGeometryGroupDestroy(geometrytrianglesgroup) );
#endif
...
#if defined(ORT_USERTXAPIS)
    // OptiX RTX hardware-accelerated triangles API
#if (OPTIX_VERSION >= 60500)
    // XXX Destroying the RTX triangle instances can cause crashes,
    //     both in the OptiX 5.2 DEV build w/ triangle API 0.3,
    //     and also the production OptiX 6.0.0 build w/ driver 418.30.
    //     This may be caused by an interaction between our use of
    //     RT_GEOMETRY_BUILD_FLAG_RELEASE_BUFFERS in the triangle APIs
    //     and the instance nodes.
    //     As of OptiX 6.5.0 and driver version 440.44, this problem 
    //     seems to be corrected, and this workaround is no longer needed.
...
void OptiXRenderer::update_rendering_state(int interactive) {
...
#if defined(ORT_USERTXAPIS)
  // Permit VMD usage of the RTX hardware triangle API to 
  // be disabled at runtime, for debugging/comparison purposes.
  // This does not affect RTX execution mode, but rather VMD's 
  // internal usage of the RTX-specific triangle APIs.
  ...
  if (getenv("VMDOPTIXSCENEEPSILON") != NULL) {
    float scene_epsilon = float(atof(getenv("VMDOPTIXSCENEEPSILON")));
...
//    RTERR( rtAccelerationSetBuilder(acceleration, "Bvh") );
  ...
  if (getenv("VMDOPTIXAOMAXDIST")) {
    float tmp = float(atof(getenv("VMDOPTIXAOMAXDIST")));
    if (verbose == RT_VERB_DEBUG) {
  ...
  //
  ext_aa_loops = 1;
  int totrays = (aa_samples + 1) * ((ao_samples > 0) ? ao_samples : 1);
  int maxrayspass = 16;
  if (getenv("VMDOPTIXMAXRAYSPERPASS") != NULL) {
    maxrayspass = atoi(getenv("VMDOPTIXMAXRAYSPERPASS"));
    printf("OptiXRenderer) User-override of max rays per pass: %d\n", maxrayspass);
  }
  if ((getenv("VMDOPTIXFORCEMULTIPASS") != NULL) || (totrays > maxrayspass)) {
  ...
  // Check/update OptiX context-wide stack size calculations
  // based on required recursion depth, shader complexity, etc.
  int stacksizeadjusted=0;

#if (OPTIX_VERSION >= 60000)
  // When running in RTX mode, OptiX >= 6.x provides an API to set
  // the maximum trace depth which is used to automatically compute the
  // required stack size.
  if (rtx_enabled) {
    // We need to tell OptiX to allow a recursion depth that includes
    // not only the surface hit, but also shadow feelers and any other
    // secondary rays that aren't being tracked explicitly by the 
    // internal recursion depth counter implemented in the top level 
    // ray gen and shading code.
    int newmaxdepth = scene_max_depth + 1;

    // A last-ditch workaround for issues observed in the field where
    // the requested max trace depth somehow didn't provide sufficient
    // stack space in practice.  This can be used for force OptiX to 
    // use the maximum stack size allowed by the RTX runtime.
    if (getenv("VMDOPTIXMAXSTACKSIZE")) {
      printf("OptiXRenderer) Setting RTX runtime to max depth / stack size\n");

      // The maximum recursion depth supported by OptiX 6.0 is 31.
      newmaxdepth = 31;
    }

    // Enforce maximum supported trace depth limit.
    // The maximum recursion depth supported by OptiX 6.0 is 31.
    if (newmaxdepth > 31)
      newmaxdepth = 31;

    RTERR( rtContextSetMaxTraceDepth(ctx, newmaxdepth) );
    RTERR( rtContextSetMaxCallableProgramDepth(ctx, newmaxdepth) );

    stacksizeadjusted = 1;
  }
#endif

  // When not running in RTX mode, we revert to the old OptiX [1-5.x]
  // stack size APIs to set the stack size required for VMD scenes
  // with a typical recursion depth.
  // XXX The rtContext[GS]etStackSize() APIs are not supported when
  //     using the RTX execution strategy.  Although they don't (yet)
  //     return errors in that case, we protect against calling them here
  //     except when running in a non-RTX mode.  Presumably these APIs will
  //     be deprecated in future OptiX releases.
  if (!stacksizeadjusted) {
...
#if !defined(VMDOPTIX_PROGRESSIVEAPI)
  // Set print buffer size when using the old non-progressive APIs 
...
void OptiXRenderer::framebuffer_config(int fbwidth, int fbheight,
                                       int interactive) {
  if (!context_created)
    return;
  ...
  if (buffers_allocated) {
    ...
    RTERR( rtBufferCreate(ctx, RT_BUFFER_OUTPUT, &raystats1_buffer) );
    ...
    RTERR( rtBufferCreate(ctx, RT_BUFFER_OUTPUT, &raystats2_buffer) );
...
void OptiXRenderer::framebuffer_resize(int fbwidth, int fbheight) {
  if (!context_created)
    return;
  ...
  if (buffers_allocated) {
    if (verbose == RT_VERB_DEBUG) 
      printf("OptiXRenderer) framebuffer_resize(%d x %d)\n", width, height);
...
void OptiXRenderer::framebuffer_map_rgb4u(unsigned char **imgrgb4u) {
  rtBufferMap(framebuffer, (void **) imgrgb4u);
}
...
void OptiXRenderer::framebuffer_destroy() {
  if (!context_created)
    return;
...
//  const float msr_warp_coeff[5] = { 1.000f, 0.290f, 0.195f, 0.045f, 0.360f };
  ...
  if (getenv("VMDOPTIXHMDFOV")) {
    hmd_fov = float(atof(getenv("VMDOPTIXHMDFOV")));
  ...
  framebuffer_config(width, height, 1);
  ...
  if (getenv("VMDOPTIXNORTX") != NULL || rtx_enabled==0) {
  ...
  double accumbufstarttime=wkf_timer_timenow(ort_timer); 
        ...
        switch (eventtype) {
          case VideoStream::VS_EV_ROTATE_BY:
            { int axis;
              ...
              switch (axis) {
                case 'x':
                  rm.rotate_axis(cam_U, -angle * VMD_PI/180.0f);
                  break;

                case 'y':
                  rm.rotate_axis(cam_V, -angle * VMD_PI/180.0f);
                  break;

                case 'z':
                  rm.rotate_axis(cam_W, -angle * VMD_PI/180.0f);
                  break;
              }
    ...
    if (spaceballenabled) {
      ...
      if (glwin_get_spaceball(win, &rx, &ry, &rz, &tx, &ty, &tz, &buttons)) {
        ...
        if (spaceballmode == 0) {
          ...
          float divlen = sqrtf(wsx*wsx + wsy*wsy) * 50.0f;
...
//      render_compile_and_validate();
      ...
      if (lasterror == RT_SUCCESS) {
        if (winredraw) {
          ...
          accumbufstarttime=wkf_timer_timenow(ort_timer);
#endif
#ifdef VMDOPTIX_PROGRESSIVEAPI
          ...
          if (frame_ready || hmd_freerun) {
            ...
            if (movie_recording_enabled && movie_recording_on) {
              ...
              for (symidx=movie_lastframeindex; symidx<fidx; symidx++) {
                ...
                if (symlink(moviefilename, symlinkfilename) < 0)
                  perror("symlink: ");
              }
#endif
  ...
  if (lasterror == RT_SUCCESS) {
#if defined(ORT_RAYSTATS)
    double frametime = wkf_timer_timenow(ort_timer) - accumbufstarttime;
    OptiXPrintRayStats(raystats1_buffer, raystats2_buffer, frametime);
#endif

    wkf_timer_start(ort_timer);
    OptiXWriteImage(filename, writealpha, framebuffer); // write output image
...
void OptiXRenderer::render_to_file(const char *filename, int writealpha) {
  ...
  framebuffer_config(width, height, 0);
...
void OptiXRenderer::add_material_cmdlist(const VMDDisplayList *cmdList) {
  add_material(cmdList->materialtag,
               cmdList->ambient,
               cmdList->diffuse,
               cmdList->specular,
               cmdList->shininess,
               ...
               cmdList->opacity,
               cmdList->outline,
               cmdList->outlinewidth,
               cmdList->transmode);
}


void OptiXRenderer::scene_aggregate_cmdlist(const VMDDisplayList *cmdList,
                                            const float *colorData) {
//  printf("OptiXRenderer) --> RTX RTRT Passthrough\n");
  ...
  Stack<Matrix4> transMat(10);
  Matrix4 m;
  transMat.push(m);           // push on the identity matrix
  (transMat.top()).multmatrix(cmdList->mat);   // save transformation matrix
...
//  textMat.multmatrix(cmdList->mat);
  ...
  while((tok = cmdList->next(&cmditer, cmdptr)) != DLASTCOMMAND) {
    switch (tok) {
      case DCOLORINDEX:
        colorIndex = (((DispCmdColorIndex *)cmdptr)->color);
        break;
      ...
      case DCYLINDER: // plot a cylinder
        ...
        // XXX these should be getting aggregated in a batch

        float points[6], radii[1], colors[3];
        memcpy(points,           (float *)cmdptr, 3L*sizeof(float));
        memcpy(&points[3], ((float *)cmdptr) + 3, 3L*sizeof(float));
        radii[0] = ((float *)cmdptr)[6];
        int filled = ((int) ((float *) cmdptr)[8]); 
        memcpy(colors, &colorData[colorIndex*3L], 3L*sizeof(float));

        float *mat =  &transMat.top().mat[0];
        float scaleFactor =
          (sqrtf(mat[0]*mat[0] + mat[4]*mat[4] + mat[ 8]*mat[ 8]) +
        ...
        cylinder_array_color(&transMat.top(), scaleFactor,
                             1, points, radii, colors,
                             cmdList->materialtag);

        // XXX no filled cylinder caps yet
      ...
      case DCYLINDERARRAY:
        ...
        DispCmdCylinderArray *ca = (DispCmdCylinderArray *)cmdptr;
        float *points, *radii, *colors;
        ca->getpointers(points, radii, colors);
        ...
        // Submit all geometry in one buffer
        cylinder_array_color(&transMat.top(), scaleFactor,
                             ca->numcylinders, points, radii, colors,
                             cmdList->materialtag);
        }
        break;
      ...
      case DSPHERE:
        ...
        float centers[3], radii[1], color[3];
        memcpy(centers, (float *)cmdptr, 3L*sizeof(float));
        radii[0] = ((float *)cmdptr)[3];
        ...
        // Submit all geometry in one buffer
        sphere_array_color(transMat.top(), scaleFactor,
                           1, centers, radii, &colorData[colorIndex*3L],
                           cmdList->materialtag);
        } 
        break;
      ...
      case DSPHEREARRAY:
        ...
        // Submit all geometry in one buffer
        sphere_array_color(transMat.top(), scaleFactor,
                           sa->numspheres, centers, radii, colors,
                           cmdList->materialtag);
        }
        break;
...
#if 1
          printf("Dropped trimesh_c3f_n3f_v3f()\n");
#else
          // XXX unimplemented
          trimesh_c3f_n3f_v3f(transMat.top(), c, n, v, cmd->numfacets, 
                              cmdList->materialtag);
#endif
        } else if (cmd->pervertexnormals) {
          cmd->getpointers(n, v);
          trimesh_n3f_v3f(transMat.top(), &colorData[colorIndex * 3],
                          n, v, cmd->numfacets, cmdList->materialtag);
        } else {
          cmd->getpointers(n, v);
          trimesh_v3f(transMat.top(), &colorData[colorIndex * 3],
                      v, cmd->numfacets, cmdList->materialtag);
        }
        }
        break;
      ...
      case DTRIMESH_C4F_N3F_V3F:
        ...
        trimesh_c4n3v3(transMat.top(), cmd->numverts, cnv, cmd->numfacets, f,
                       cmdList->materialtag);
        }
        break;
      ...
      case DTRIMESH_C4U_N3B_V3F: // draw a triangle mesh
        ...
        if (cmd->pervertexcolors) {
          cmd->getpointers(c, n, v);
          trimesh_c4u_n3b_v3f(transMat.top(), c, n, v, cmd->numfacets, 
                              cmdList->materialtag);
        } else {
          cmd->getpointers(n, v);
          trimesh_n3b_v3f(transMat.top(), &colorData[colorIndex * 3],
                          n, v, cmd->numfacets, cmdList->materialtag);
        }
        }
        break;
      ...
      case DTRISTRIP:     // draw a triangle strip
        ...
        tristrip(transMat.top(), cmd->numverts, cnv, cmd->numstrips, 
                 vertsperstrip, f, cmdList->materialtag);
        }
        break;
...
/// finalize scene and be prepared to render it...
void OptiXRenderer::scene_aggregation_complete() {
  update_rendering_state(0);
  render_compile_and_validate();
}


// launch rendering pass(es)
void OptiXRenderer::render_current_scene() {
  if (lasterror == RT_SUCCESS) {
    // clear the accumulation buffer
    RTERR( rtContextLaunch2D(ctx, RT_RAY_GEN_CLEAR_ACCUMULATION_BUFFER, width, height) );
...
void OptiXRenderer::set_material(RTgeometryinstance instance, int matindex, 
                                 const float *uniform_color, int hwtri) {
  if (!context_created)
    return;
...
/// Compute uniform scaling factor for cone/cylinder/sphere primitives
/// if/when flattening the scene graph for best interactive performance.
float OptiXRenderer::calc_matrix_scale_factor(const float *mat) {
  float scaleFactor =
    (sqrtf(mat[0]*mat[0] + mat[4]*mat[4] + mat[ 8]*mat[ 8]) +
     sqrtf(mat[1]*mat[1] + mat[5]*mat[5] + mat[ 9]*mat[ 9]) +
     sqrtf(mat[2]*mat[2] + mat[6]*mat[6] + mat[10]*mat[10])) / 3.0f;
...
void OptiXRenderer::ring_array_color(Matrix4 & wtrans, float rscale,
                                     int rnum, const float *centers,
                                     const float *norms, const float *radii, 
                                     const float *colors, int matindex) {
  if (!context_created) return;
...
void OptiXRenderer::sphere_array_color(Matrix4 & wtrans, float rscale,
                                       int spnum, const float *centers,
                                       const float *radii, const float *colors,
                                       int matindex) {
  if (!context_created) return;
...
//if (verbose == RT_VERB_DEBUG) printf("OptiXRenderer) creating tricolor list: %d...\n", numtris);
  ...
  RTERR( rtGeometryTrianglesSetPrimitiveCount(geom_hwtri, tcnt) );
  RTERR( rtGeometryTrianglesSetVertices(geom_hwtri, tcnt * 3, vbuf, 
                                        0, sizeof(float3), RT_FORMAT_FLOAT3) );
#if defined(VMDOPTIXRTXRELEASEBUFS)
  RTERR( rtGeometryTrianglesSetBuildFlags(geom_hwtri, RT_GEOMETRY_BUILD_FLAG_RELEASE_BUFFERS) );
#else
  append_buffer(vbuf);
#endif
...
void OptiXRenderer::tricolor_list(Matrix4 & wtrans, int numtris, 
                                  const float *vnc, int matindex) {
  if (!context_created) return;
//if (verbose == RT_VERB_DEBUG) printf("OptiXRenderer) creating tricolor list: %d...\n", numtris);
  tricolor_cnt += numtris;
...
#if defined(ORT_USERTXAPIS)
void OptiXRenderer::trimesh_c4n3v3_hwtri(Matrix4 & wtrans, 
...
#if defined(ORT_USERTXAPIS)
void OptiXRenderer::trimesh_v3f_hwtri(Matrix4 & wtrans, 
                                      const float *uniform_color, 
                                      const float *v, 
                                      int numfacets, int matindex) {
  if (!context_created) return;
...
void OptiXRenderer::trimesh_v3f(Matrix4 & wtrans, const float *uniform_color, 
