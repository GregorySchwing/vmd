 ...
 *	$Revision: 1.74 $	$Date: 2019/11/07 03:52:53 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
Wavefunction::Wavefunction(int ncoeffs,
  ...
  orb_id2index(NULL)
{
  strncpy(info, infostr, QMDATA_BUFSIZ - 1);
                                 ...
                                 int &num_signa_ts) 
{
  ...
  memset(static_cast<void *>(newwavef), 0, (num_wavef+1)*sizeof(Wavefunction));
  int i;
  for (i=0; i<num_wavef; i++) {
  ...
  if (info) 
    strncpy(wavef[iwave].info, info, QMDATA_BUFSIZ-1);
  else 
