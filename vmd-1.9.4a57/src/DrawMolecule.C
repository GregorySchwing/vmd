 ...
 *	$Revision: 1.146 $	$Date: 2019/04/12 04:41:53 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
int DrawMolecule::need_secondary_structure(int calc_if_not_yet_done) {
  ...
  if (calc_if_not_yet_done) {
    ...
    int usestride=1;
    if (getenv("VMDUSEDSSP") != NULL) {
      usestride=0;
    }

    if (!usestride) {
      msgInfo << "User override of STRIDE, using DSSP for secondary structure"
              << sendmsg;
      if (ss_from_dssp(this)) {
        msgErr << "Call to DSSP program failed." << sendmsg;
        return FALSE;
      }
      return TRUE;
    } 
