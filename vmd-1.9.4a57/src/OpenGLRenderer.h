 ...
 *	$Revision: 1.154 $	$Date: 2021/10/08 03:39:38 $
 *
 ***************************************************************************/
/**
...
#define OPENGL_RENDER_RTXRTRT              2
...
#if defined(VMDOPTIXRTRT)
class OptiXRenderer;  //<! forward declaration of classes here
#endif
...
protected:
  ...
  GLfloat ogl_backcolor[4];       // background color
  GLfloat ogl_backgradient[2][4]; // background gradient colors
  int     ogl_acrobat3dcapture;   // don't cache anything, for 3-D capture
  //@}

#if defined(VMDOPTIXRTRT)
  /// full-time interactive RT state
  OptiXRenderer * ort;            // OptiXRenderer context
  int ogl_optix_rtrt_passthrough;    // when active, RTX RTRT does rendering
#endif
...
public:
  ...
  virtual void set_window_title(char *) {};        ///< set window title bar str
};
