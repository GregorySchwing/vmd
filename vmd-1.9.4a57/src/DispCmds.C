 ...
 *	$Revision: 1.121 $	$Date: 2021/05/14 22:47:22 $
 *
 ***************************************************************************/
/**
 *  \file DispCmds.C
 *  \brief Display commands that marshall, pack, and store scene geometry 
 *  in a VMDDisplayList object.
 * 
 *  The original display list implementation in VMD was heavily stateful,
 *  (mirroring early IrisGL and OpenGL) which leads to unnecessary complexity,
 *  limited opportunities for exploiting parallel algorithms and parallel
 *  rendering, and a noteworthy exposure to strange and rare bugs related to
 *  persistent state management.  Going forward, we want to resolve these
 *  lingering issues and this will require conversion of the old display
 *  commands and their packed representations to be more self-contained,
 *  without reference to significant external rendering state.  This will
 *  directly permit parallel rendering of individual display tokens and
 *  yield inherently more localized and optimal loops in the back-end
 *  rendering code since there will no longer need to be concern for
 *  saving or restoring rendering state pre/post a given display command.
 *  This is an ongoing goal and the present implementation is still far from
 *  achieving it.
 */
...
void DispCmdTriMesh::putdata(const float * vertices,
  ...
  if (enablestrips)  {
    ...
    ptrdiff_t fsize = num_facets * 3L;
    ptrdiff_t i, ind, ii;
    ptrdiff_t iPrimCount = 0;
    ptrdiff_t iCurrPrimSize;
...
void DispCmdCylinderArray::putdata(const float * cylpoints,
                                   const float * cylradii,
                                   const float * cylcolors,
                                   int num_cyls, int cyl_res, int cyl_caps,
                                   VMDDisplayList * dobj) {

  DispCmdCylinderArray *ptr = (DispCmdCylinderArray *) dobj->append(DCYLINDERARRAY, 
                           sizeof(DispCmdCylinderArray) +
                           sizeof(float) * num_cyls * 6L +
                           sizeof(float) * num_cyls + 
                           sizeof(float) * num_cyls * 3L +
                           sizeof(int) * 3L);
  if (ptr == NULL)
    return;
  ptr->numcylinders = num_cyls;
  ptr->cylinderres = cyl_res;
  ptr->cylindercaps = cyl_caps;

  float *points;
  float *radii;
  float *colors;
  ptr->getpointers(points, radii, colors);

  memcpy(points, cylpoints, sizeof(float) * num_cyls * 6L);
  memcpy(radii, cylradii, sizeof(float) * num_cyls);
  memcpy(colors, cylcolors, sizeof(float) * num_cyls * 3L);
}
#endif
...
void DispCmdCone::putdata(const float *p1, const float *p2,
                          float newrad, float newrad2, int newres,
                          VMDDisplayList *dobj) {
...
void DispCmdText::putdata(const float *c, const char *s, float thickness, 
                          float size, float ox, float oy, VMDDisplayList *dobj) {
  if (s != NULL) {
    size_t len = strlen(s)+1;
    char *buf = (char *)(dobj->append(DTEXT, len+7L*sizeof(float)));
    if (buf == NULL) 
      return;
    ((float *)buf)[0] = c[0];          // X
    ((float *)buf)[1] = c[1];          // Y
    ...
    ((float *)buf)[4] = size;          // size
    ((float *)buf)[5] = ox;            // X offset
    ((float *)buf)[6] = oy;            // Y offset
    memcpy(buf+7L*sizeof(float),s,len); // text string
  }
}
...
void DispCmdPickPointArray::putdata(int num, int numsel, int firstsel, int *on, 
  ...
  if (num == numsel) {
    ...
    ptrdiff_t cnt=numsel; // early-exit as soon as we found the last selected atom
    ptrdiff_t i, cp;
