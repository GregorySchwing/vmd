 ...
 *	$Revision: 1.33 $	$Date: 2021/12/21 23:02:14 $
 *
 ***************************************************************************/
/**
 *  \file OpenGLShader.C
 *  \brief Hardware query, JIT compilation, and runtime management 
 *  of VMD's OpenGL programmable shaders.
 */
...
int OpenGLShader::CompileShaders(GLubyte *vertexShader, GLubyte *fragmentShader) {
  ...
  length = GLint(strlen((const char *) vertexShader));
  ...
  length = GLint(strlen((const char *) fragmentShader));
