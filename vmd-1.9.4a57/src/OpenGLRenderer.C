 ...
 *	$Revision: 1.478 $	$Date: 2021/12/21 23:12:43 $
 *
 ***************************************************************************/
/**
 *  \file OpenGLRenderer.C
 *  \brief Rendering-only DisplayDevice subclass that performs 
 *  core OpenGL rendering operations to generate molecular graphics from 
 *  VMDDisplayList tokens. 
 * 
 *  OpenGLDisplayDevice is not a complete DisplayDevice subclass definition. 
 *  Another subclass must provide routines to open windows, reshape, clear, 
 *  set perspective, etc.  This object only contains the low-level OpenGL
 *  code to render VMDDisplayList DispCmd tokens to create molecular graphics.
 *  OpenGLRenderer renders drawing commands into a framebuffer and (possibly
 *  in an on-screen window) managed by one of the other DisplayDevice 
 *  subclasses, e.g., OpenGLDisplayDevice, FltkDisplayDevice, 
 *  OpenGLPbufferDisplayDevice, etc.
 */
...
static void vmd_DrawSphere(float rad, int res, int solid) {
  ...
  // longitudinal "slices"
  float ang_twopi_res = VMD_TWOPIF / res;
  for (i=0; i<res; i++) {
    float angle = i * ang_twopi_res;
    sincosf(angle, &sinLong[i], &cosLong[i]);
  }
  ...
  // latitude "stacks"
  float ang_pi_res = VMD_PIF / res;
  for (i=0; i<=res; i++) {
    float angle = i * ang_pi_res;
    sincosf(angle, &sinLatNorm[i], &cosLatNorm[i]);
...
static void vmd_DrawConic(float *base, float *apex, float radius, float radius2, int numsides) {
  ...
  if (radius2 > 0) {
    ...
    for (h=0; h <= numsides+3; h++) {
      ...
      float projx = (cosTheta*perp[0] + sinTheta*perp2[0]);
      float projy = (cosTheta*perp[1] + sinTheta*perp2[1]);
      float projz = (cosTheta*perp[2] + sinTheta*perp2[2]);

      offsetL[0] = radius2 * projx;
      offsetL[1] = radius2 * projy;
      offsetL[2] = radius2 * projz;

      offsetT[0] = radius  * projx;
      offsetT[1] = radius  * projy;
      offsetT[2] = radius  * projz;
    ...
    for (h=0; h < numsides+3; h++) {
      ...
      vert0[0] = base[0] + radius * projx;
      vert0[1] = base[1] + radius * projy;
      vert0[2] = base[2] + radius * projz;
...
void OpenGLRenderer::set_background(const float *newback) {
  // make softcopy of background color for RTRT mode etc
  ogl_backcolor[0] = (GLfloat) newback[0];
  ogl_backcolor[1] = (GLfloat) newback[1];
  ogl_backcolor[2] = (GLfloat) newback[2];
  ogl_backcolor[3] = (GLfloat) 1.0f;

  // set fog color used for depth cueing
  glFogfv(GL_FOG_COLOR, ogl_backcolor);

  // set clear color
  glClearColor((GLclampf) ogl_backcolor[0],
               (GLclampf) ogl_backcolor[1],
               (GLclampf) ogl_backcolor[2], 1.0);
}
...
void OpenGLRenderer::set_render_mode(int newMode) {
  ...
  switch (renderMode) {
    case OPENGL_RENDER_NORMAL:
      ...
      ogl_acrobat3dcapture = 0;
#if defined(VMDOPTIXRTRT)
      ogl_optix_rtrt_passthrough = 0;
#endif
      strcat(wtitle, " (OpenGL)");
      break;
    ...
    case OPENGL_RENDER_ACROBAT3D:
      ...
      ogl_optix_rtrt_passthrough = 0;
#endif
      strcat(wtitle, " (OpenGL - Acrobat3D)");
      break;


#if defined(VMDOPTIXRTRT)
    case OPENGL_RENDER_RTXRTRT:
#if defined(VMDUSEOPENGLSHADER)
      ...
      ogl_useglslshader = 0;
      ogl_acrobat3dcapture = 0;
      ogl_optix_rtrt_passthrough = 1;
      strcat(wtitle, " (Tachyon - OptiX RTX)");
      break;
#endif
...
#if defined(VMDOPTIXRTRT)
  if (ogl_optix_rtrt_passthrough) {
#if 0
    printf("OpenGLRenderer::prepare3D(%ld x %ld) --> RTX RTRT Passthrough\n",
           xSize, ySize);
#endif
    ort->destroy_scene();
    ort->framebuffer_config(xSize, ySize, 0);
//    ort->framebuffer_resize(xSize, ySize);

    ort->shadows_on(shadows_enabled()); // shadowing mode required 
    ort->set_ao_samples(ao_enabled() * 8);
    ort->set_ao_ambient(get_ao_ambient());
    ort->set_ao_direct(get_ao_direct());
    ort->dof_on(dof_enabled());
    ...
    for (i=0; i<DISP_LIGHTS; i++) {
      if (ogl_lightstate[i]) {
        ort->add_directional_light(ogl_lightpos[i], ogl_lightcolor[i]);
      }
    }
    ...
    // set camera projection mode and associated background mode
    switch (projection()) {
    ...
    ort->set_bg_color(ogl_backcolor);

    // Specify Tachyon/OptiX background sky sphere/plane if gradient mode is on
    if (backgroundmode == 1) {
      ...
      switch (projection()) {
        case DisplayDevice::ORTHOGRAPHIC:
          ...
          // the incident ray origin and the sky plane gradient "up" vector,
      ...
      // set background color, fog color, and background gradient colors
      ort->set_bg_color_grad_top(ogl_backgradient[0]);
      ort->set_bg_color_grad_bot(ogl_backgradient[1]);
...
#if defined(VMDOPTIXRTRT)
  /// If real-time ray tracing pass-through is enabled, we process the
  /// current group of VMDDisplayList DispCmd tokens, aggregating them
  /// into the retained RT scene.
  if (ogl_optix_rtrt_passthrough) {
    ort->scene_aggregate_cmdlist(cmdList, colorData);
    return;
  }
#endif
...
#define STRIPEWIDTH 32
      ...
      case DCYLINDERARRAY:
        ...
        int i,ind3,ind6;
        for (i=0,ind3=0,ind6=0; i<ca->numcylinders; i++,ind3+=3,ind6+=6) {
          glColor3fv(&colors[ind3]);
//        cylinder_full(ca->cylinderres, points+ind6, radii[i], ca->cylindercaps);
          cylinder(points + ind6, points + ind6+3, ca->cylinderres,
                   radii[i], radii[i]);
//, ca->cylindercaps);
        }
        }
      ...
      case DTEXT:
        ...
        GLfloat textsize = pos[4];
        float textoffset_x = pos[5];
        float textoffset_y = pos[6];
        char *txt = (char *)(pos+7);
        float wp[4];
        float mp[4] = { 0, 0, 0, 1};
...
// #define VMDMSAAFONTTOGGLE 1
            ...
            glCallLists(GLsizei(strlen(txt)), GL_UNSIGNED_BYTE, (GLubyte *)txt);
...
#if defined(VMDOPTIXRTRT)
  if (ogl_optix_rtrt_passthrough) {
//printf("OpenGLRenderer::render_done()\n");

    ort->scene_aggregation_complete(); // if 1st time, complete rendering setup

    ort->render_current_scene();

    unsigned char *imgrgb4u=NULL;
    ort->framebuffer_map_rgb4u(&imgrgb4u);
...
//      glShadeModel(GL_FLAT);
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, xs, ys, 0,
                   GL_RGBA, GL_UNSIGNED_BYTE, imgrgb4u);
      ...
      glVertex2f(0, GLfloat(ys));
      glTexCoord2f(1.0f, 1.0f);
      glVertex2f(GLfloat(xs), GLfloat(ys));
      glTexCoord2f(1.0f, 0.0f);
      glVertex2f(GLfloat(xs), 0.0f);
      glEnd();
      ...
      glLoadIdentity();
      glRasterPos2i(0, 0);
      glDrawPixels(xs, ys, GL_RGBA, GL_UNSIGNED_BYTE, imgrgb4u);
#endif
