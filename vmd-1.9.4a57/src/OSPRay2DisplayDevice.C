...
* RCS INFORMATION:
*
*      $RCSfile: OSPRay2DisplayDevice.C,v $
*      $Author: johns $      $Locker:  $               $State: Exp $
*      $Revision: 1.5 $         $Date: 2021/12/13 07:54:00 $
*
***************************************************************************/
/**
 *  \file OSPRay2DisplayDevice.C
...
int OSPRay2DisplayDevice::OSPRay_Global_Init(void) {
  OSPRay2Renderer::OSPRay_Global_Init(); // call only ONCE
}

// Global OSPRay shutdown routine -- call it only ONCE...
void OSPRay2DisplayDevice::OSPRay_Global_Shutdown(void) {
  OSPRay2Renderer::OSPRay_Global_Shutdown(); // call only ONCE
}


/// constructor ... initialize some variables
OSPRay2DisplayDevice::OSPRay2DisplayDevice(VMDApp *app, int interactive) : FileRenderer((interactive) ? 
  ...
  ort = new OSPRay2Renderer();
  ort_timer = wkf_timer_create();
}
        
/// destructor
OSPRay2DisplayDevice::~OSPRay2DisplayDevice(void) {
  delete ort;
  wkf_timer_destroy(ort_timer);
}

void OSPRay2DisplayDevice::add_material(void) {
...
void OSPRay2DisplayDevice::reset_vars(void) {
...
void OSPRay2DisplayDevice::send_cylinder_buffer() {
...
// draw a cylinder
void OSPRay2DisplayDevice::cylinder(float *a, float *b, float r, int filled) {
...
// draw a sphere array
void OSPRay2DisplayDevice::sphere_array(int spnum, int spres, float *centers, 
...
//
void OSPRay2DisplayDevice::text(float *pos, float size, float thickness,
                              const char *str) {
  float textpos[3];
...
void OSPRay2DisplayDevice::send_triangle_buffer() {
...
// draw a triangle
void OSPRay2DisplayDevice::triangle(const float *a, const float *b, const float *c, const float *n1, const float *n2, const float *n3) {
  // if we have a change in transformation matrix, color, or material state,
...
// draw a tricolor
void OSPRay2DisplayDevice::tricolor(const float *a, const float *b, const float *c,
...
void OSPRay2DisplayDevice::trimesh_c4u_n3b_v3f(unsigned char *c, signed char *n,
...
void OSPRay2DisplayDevice::trimesh_c4u_n3f_v3f(unsigned char *c, float *n,
                                             float *v, int numfacets) {
  add_material();
  ort->trimesh_c4u_n3f_v3f(transMat.top(), c, n, v, numfacets, materialIndex);
}

void OSPRay2DisplayDevice::trimesh_c4n3v3(int numverts, float * cnv,
...
void OSPRay2DisplayDevice::trimesh_n3b_v3f(signed char *n, float *v, int numfacets) {
  add_material();
  ort->trimesh_n3b_v3f(transMat.top(), matData[colorIndex], n, v, numfacets,  materialIndex);
}

void OSPRay2DisplayDevice::trimesh_n3f_v3f(float *n, float *v, int numfacets) {
  add_material();
...
#if 0
void OSPRay2DisplayDevice::trimesh_n3fopt_v3f(float *n, float *v, int numfacets) {
  add_material();
...
void OSPRay2DisplayDevice::tristrip(int numverts, const float * cnv,
...
void OSPRay2DisplayDevice::write_lights() {
  int i;
  int lightcount = 0;
  ...
  for (i=0; i<DISP_LIGHTS; i++) {
    if (advLightState[i].on) {
      ...
      if (advLightState[i].spoton) {
        printf("OSPRay2DisplayDevice) SpotLight not implemented yet ...\n");
      } else {
        apitexture tex;
...
void OSPRay2DisplayDevice::write_materials() {
  ...
  if (backgroundmode == 1) {
    ...
    if (projection() == DisplayDevice::ORTHOGRAPHIC)
      ort->set_bg_mode(OSPRay2Renderer::RT_BACKGROUND_TEXTURE_SKY_ORTHO_PLANE);
    else
      ort->set_bg_mode(OSPRay2Renderer::RT_BACKGROUND_TEXTURE_SKY_SPHERE);
    ...
    ort->set_bg_mode(OSPRay2Renderer::RT_BACKGROUND_TEXTURE_SOLID);
  }
}
...
void OSPRay2DisplayDevice::write_header() {
  ...
  if (cueingEnabled) {
    switch (cueMode) {
      case CUE_LINEAR:
        ort->set_cue_mode(OSPRay2Renderer::RT_FOG_LINEAR, start, end, density);
        break;

      case CUE_EXP:
        ort->set_cue_mode(OSPRay2Renderer::RT_FOG_EXP, start, end, density);
        break;

      case CUE_EXP2:
        ort->set_cue_mode(OSPRay2Renderer::RT_FOG_EXP2, start, end, density);
        break;
    ...
    ort->set_cue_mode(OSPRay2Renderer::RT_FOG_NONE, start, end, density);
  }
}


void OSPRay2DisplayDevice::write_trailer(void){
#if 1
  ...
  switch (projection()) {
    case DisplayDevice::ORTHOGRAPHIC:
      ort->set_camera_projection(OSPRay2Renderer::RT_ORTHOGRAPHIC);
      ort->set_camera_zoom(0.5f / (1.0 / (vSize / 2.0)));
      break;

    case DisplayDevice::PERSPECTIVE:
    default:
      ort->set_camera_projection(OSPRay2Renderer::RT_PERSPECTIVE);
  ...
  if (verbstr != NULL) {
    if (!strupcmp(verbstr, "TIMING") || !strupcmp(verbstr, "DEBUG")) {
      ...
      printf("OSPRay2DisplayDevice) scene graph construction time %.2f\n",
             time_scene_graph);
    }
  }
  ...
  if (getenv("VMDOSPRAYNODESTROYCONTEXT") == NULL) {
    ...
    // make a new OSPRay2Renderer object so we're ready for the next run...
    ort = new OSPRay2Renderer();
  } else {
  ...
  printf("OSPRay2DisplayDevice) Total rendering time: %.2f sec\n", wkf_timer_time(ort_timer));
...
void OSPRay2DisplayDevice::start_clipgroup(void) {
  int i;
  int planesenabled = 0;
  ...
  if (planesenabled > 0) {
    ...
    for (i=0; i<VMD_MAX_CLIP_PLANE; i++) {
      if (clip_mode[i] > 0) {
        float ospray_clip_center[3];
        float ospray_clip_normal[3];
        float ospray_clip_distance;

        inclipgroup = 1; // we're in a clipping group presently

        // Transform the plane center and the normal
        (transMat.top()).multpoint3d(clip_center[i], ospray_clip_center);
        (transMat.top()).multnorm3d(clip_normal[i], ospray_clip_normal);
        vec_negate(ospray_clip_normal, ospray_clip_normal);

        // Tachyon uses the distance from the origin to the plane for its
        // representation, instead of the plane center
        ospray_clip_distance = dot_prod(ospray_clip_normal, ospray_clip_center);

        planes[j * 4    ] =  ospray_clip_normal[0];
        planes[j * 4 + 1] =  ospray_clip_normal[1];
        planes[j * 4 + 2] = -ospray_clip_normal[2];
        planes[j * 4 + 3] =  ospray_clip_distance;

//        ort->clip_fv(planesenabled, planes); // add the clipping planes
        j++;
      }
    }
...
void OSPRay2DisplayDevice::end_clipgroup(void) {
  if (inclipgroup) {
//    ort->clip_off();      // disable clipping planes
