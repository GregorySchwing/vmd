 ...
 *	$Revision: 1.63 $	$Date: 2020/08/14 18:58:35 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
public:
  ...
  SymbolTable(VMDApp *vmdapp) : app(vmdapp) {};
  ~SymbolTable(void);
  ...
  void add_keyword(const char *visible,
    ...
    // if the name already exists, overwrite it with the new one
    int ind = fctns.typecode(visible);
    if (ind < 0) {
      fctns.add_name(visible, new SymbolTableElement(get, set));
    } else {
      delete fctns.data(ind);
      fctns.set_data(ind, new SymbolTableElement(get, set));
    }
  }
  void add_keyword(const char *visible,
  ...
  void add_stringfctn(const char *visible,
    ...
    if (ind < 0) {
      ...
      delete fctns.data(ind);
      fctns.set_data(ind, new SymbolTableElement(get));
    }
  }
