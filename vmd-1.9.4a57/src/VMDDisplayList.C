 ...
 *      $Revision: 1.42 $      $Date: 2020/12/12 22:54:24 $
 *
 ***************************************************************************/
/**
 *  \file VMDDisplayList.C
 *  \brief VMDDisplayList holds all of the display commands
 *         VMD generates and interprets to render the molecular scene.
 */
...
int VMDDisplayList::get_clip_status(int i, int &mode) {
  if (i < 0 || i >= VMD_MAX_CLIP_PLANE) return 0;
  mode = clipplanes[i].mode;
  return 1;
}
