...
private:
  ...
  int rtx_enabled;                        ///< OptiX RTX mode enabled
...
public:
  ...
  /// convenience wrapper to add materials from VMDDisplayList blocks
  void add_material_cmdlist(const VMDDisplayList *cmdList);

  /// aggregate geometry, and materials into OptiX scene by 
  /// processing VMDDisplayList DispCmd tokens
  void scene_aggregate_cmdlist(const VMDDisplayList *cmdList, 
                               const float *colorData);

  /// finalize fully aggregated scene if not already...
  void scene_aggregation_complete();

  /// render the completely aggregated scene
