 ...
 *	$Revision: 1.61 $	$Date: 2019/09/26 04:58:39 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
void VMDTitle::redraw_list(void) {
  ...
  if (letterson) {
    ...
    DispCmdText txt;
    float size = 0.8f;
    txt.putdata(pos, "Theoretical and Computational Biophysics Group", 1.0f, size, 0, 0, cmdList);
    pos[1] -= 0.1f;
    txt.putdata(pos, "NIH Resource for Macromolecular Modeling and Bioinformatics", 1.0f, size, 0, 0, cmdList);
    pos[1] -= 0.1f;
    txt.putdata(pos, "University of Illinois at Urbana-Champaign", 1.0f, size, 0, 0, cmdList);
    pos[1] -= 0.1f;
    txt.putdata(pos, VMD_AUTHORS_LINE1, 1.0f, size, 0, 0, cmdList);
    pos[1] -= 0.1f;
    txt.putdata(pos, VMD_AUTHORS_LINE2, 1.0f, size, 0, 0, cmdList);
    pos[1] -= 0.1f;
    txt.putdata(pos, VMD_AUTHORS_LINE3, 1.0f, size, 0, 0, cmdList);
  }
}
