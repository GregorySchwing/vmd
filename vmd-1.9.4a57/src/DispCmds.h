...
enum { DBEGINREPGEOMGROUP, DPOINT, DPOINTARRAY, DLITPOINTARRAY, 
       ...
       DCYLINDERARRAY, 
#endif
       DSPHERE, DSPHEREARRAY, DCUBEARRAY,
       ...
       DPICKPOINT_ARRAY, DLASTCOMMAND=-1};
...
/// draw cylinders from the specified endpoint, radii, and color arrays
struct DispCmdCylinderArray {
  static void putdata(const float * points, const float * radii, 
                      const float * colors, int num_cyls, 
                      int cyl_res, int cyl_caps, VMDDisplayList * dobj);
  ...
  inline void getpointers(
    ...
    points = (float *) (rawptr + sizeof(DispCmdCylinderArray));
    radii  = (float *) (rawptr + sizeof(DispCmdCylinderArray) +
                        sizeof(float) * numcylinders * 6L);
    colors = (float *) (rawptr + sizeof(DispCmdCylinderArray) +
                        sizeof(float) * numcylinders * 6L +
                        sizeof(float) * numcylinders);
  }
...
struct DispCmdSphereArray {
  static void putdata(const float * centers, const float * radii, 
                      const float * colors, int num_spheres, 
                      int sphere_res, VMDDisplayList * dobj);
...
struct DispCmdTriMesh {
  ...
  inline void getpointers(unsigned char *&c, signed char *&n, float *&v) const {
    c = (unsigned char *)(((char *)this) + sizeof(DispCmdTriMesh));
    v = (float *) (c + numverts * 4L * sizeof(unsigned char));
    n = (signed char *) (v + 3L * numverts);
  }
  ...
  inline void getpointers(signed char *&n, float *&v) const {
...
struct DispCmdCone {
  void putdata(const float *, const float *, float, float, 
               int, VMDDisplayList *);
  float pos1[3], pos2[3]; 
...
struct DispCmdText {
  void putdata(const float *, const char *, float thickness, 
               float size, float ox, float oy, VMDDisplayList *);
};
