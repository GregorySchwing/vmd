 ...
 *      $Revision: 1.34 $       $Date: 2020/06/11 08:17:38 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
: CoorData(nm), is_input(input), begFrame(first), frameSkip(stride), 
  ...
  // ensure ts_page_alignment flag is cleared by default
  ts_page_align_sz=0;
  ...
  if (plugin->can_read_pagealigned_timesteps()) {
    ...
    ts_page_align_sz = 1; // set page alignment flag to indicate normal I/O
  }
}
