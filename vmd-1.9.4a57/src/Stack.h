 ...
 *	$Revision: 1.30 $	$Date: 2021/09/23 16:11:23 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
public:
  ...
  /// set stack to empty
  //
  // a cleaner O(1) alternative to something like...
  //   while (transMat.num()) {  stack.pop(); }
  //
  void clear(void) {
    items = 0;
    curr = data;
    // std::fill_n(data, sz, 0);
    memset(data, 0, sizeof(T)*sz);
  }
