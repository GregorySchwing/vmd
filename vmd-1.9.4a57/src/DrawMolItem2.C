 ...
 *	$Revision: 1.40 $	$Date: 2021/10/28 21:12:15 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
void DrawMolItem::draw_trace(float *framepos, float brad, int bres, int linethickness) {
  sprintf (commentBuffer,"Mol[%d] Rep[%d] Trace", mol->id(), repNumber);
...
void DrawMolItem::draw_dot_surface(float *framepos, float srad, int sres, int method) {
  ...
  // clamp range from 0 to 2
  if (method < 0) method = 0; 
  if (method > 2) method = 2;

  sprintf(commentBuffer, "Mol[%d] Rep[%d] Solvent", mol->id(), repNumber);
  cmdCommentX.putdata(commentBuffer, cmdList);
