 ...
 *	$Revision: 1.91 $	$Date: 2020/12/12 22:54:24 $
 *
 ***************************************************************************/
/**
 *  \file Displayable.h
...
#if 1
    // only trigger a display update/redraw if the clipping plane is enabled
    // at the time the change is made
    int clipmode = 0;
    rc = cmdList->get_clip_status(i, clipmode);
    if (rc && (clipmode != 0)) _needUpdate = 1;
#else
    if (rc) _needUpdate = 1;
#endif
    return rc;
...
#if 1
    // only trigger a display update/redraw if the clipping plane mode 
    // changes relative to its prior state
    int oldclipmode = 0;
    int rc = cmdList->get_clip_status(i, oldclipmode);
    if (rc && (oldclipmode != mode)) {
      rc = cmdList->set_clip_status(i, mode);
      if (rc) _needUpdate = 1;
    }
#else
