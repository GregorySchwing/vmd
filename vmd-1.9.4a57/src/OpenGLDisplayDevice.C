 ...
 *	$Revision: 1.217 $	$Date: 2021/08/24 16:52:01 $
 *
 ***************************************************************************/
/**
 * \file OpenGLDisplayDevice.C
 * \brief OpenGLRenderer subclass managing X-windows window and input state, 
...
#if defined(VMDOPTIXRTRT)
  ort = NULL; // OptiXRenderer context
...
#if defined(VMDOPTIXRTRT)
  printf("OpenGLDisplayDevice) Creating OptiX RTRT context...\n");
...
//    ort->set_bg_color(backColor);
    ...
    ort->set_aa_samples(8);
    ort->set_ao_samples(ao_enabled());
    ...
    ort->set_cue_mode(OptiXRenderer::RT_FOG_NONE, 
    ...
    // XXX what about generating a fully populated material table?
  }

  printf("OpenGLDisplayDevice) OptiX RTRT context created.\n");
#endif
...
#if defined(VMDOPTIXRTRT)
  printf("OpenGLDisplayDevice) Destroying OptiX RTRT context...\n");
  delete ort;
  ort = NULL;
  printf("OpenGLDisplayDevice) OptiX RTRT context destroyed.\n");
#endif
...
#if defined(VMDOPTIXRTRT)
  if (ort != NULL) ort->framebuffer_resize(w, h);
#endif
}
...
#if !defined(VMD_NANOHUB)
  // Normal contexts are double-buffered, but Nanohub uses an FBO that is not.
  if (do_update)
...
void OpenGLDisplayDevice::set_window_title(char *newtitlestr) {
#if !defined(VMDSDL) && !defined(_MSC_VER)
  XStoreName(glxsrv.dpy, glxsrv.windowID, newtitlestr);
#endif
}
