 ...
 *      $Revision: 1.155 $       $Date: 2022/01/21 07:25:03 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
int measure_center(const AtomSel *sel, const float *framepos,
...
#if 1
  sel->for_selected_lambda([&] (int i) {
                             float tw = weight[j];
                             w += double(tw);

                             long ind = 3L * i;
                             x += double(tw * framepos[ind  ]);
                             y += double(tw * framepos[ind+1]);
                             z += double(tw * framepos[ind+2]);
                             j++; 
                           });
#else
  auto lambda_com = [&] (int i) {
    float tw = weight[j];
    w += double(tw);
  ...
  sel->for_selected_lambda(lambda_com);
#endif
...
int measure_minmax(int num, const int *on, const float *framepos, 
  ...
  int firstsel = 0;
  int lastsel = -1;
  if (analyze_selection_aligned_dispatch(NULL, num, on, &firstsel, &lastsel, NULL))
...
extern int measure_sasa(const AtomSel *sel, const float *framepos,
  ...
  for (i=0; i<npts; i++) {
    ...
    spherepts[3L*i  ] = R*cphi;
    spherepts[3L*i+1] = R*sphi;
    spherepts[3L*i+2] = z;
  }
