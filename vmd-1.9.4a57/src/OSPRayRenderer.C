...
* RCS INFORMATION:
*
*      $RCSfile: OSPRayRenderer.C,v $
*      $Author: johns $      $Locker:  $               $State: Exp $
*      $Revision: 1.92 $         $Date: 2021/12/21 06:44:17 $
*
***************************************************************************/
/**
 *  \file OSPRayRenderer.C
...
static void vmd_ospray_error_callback(OSPError err, const char *detailstr) {
  printf("OSPRayRenderer) ERROR: '%s'\n", detailstr);
}

static void vmd_ospray_status_callback(const char *detailstr) {
  printf("OSPRayRenderer) STATUS: %s\n", detailstr);
}



// Global OSPRay initialization routine -- call it only ONCE...
int OSPRayRenderer::OSPRay_Global_Init(void) {
  DBG();
...
// Global OSPRay initialization routine -- call it only ONCE...
void OSPRayRenderer::OSPRay_Global_Shutdown(void) {
  ospShutdown();
}
...
OSPRayRenderer::OSPRayRenderer(void) {
  ...
  if (verbose == RT_VERB_DEBUG) printf("OSPRayRenderer) setting error / status callbacks...\n");
  OSPDevice dev = ospGetCurrentDevice();
  ospDeviceSetErrorFunc(dev, vmd_ospray_error_callback);
  ospDeviceSetStatusFunc(dev, vmd_ospray_status_callback);
  int loglevel = 0;
//  loglevel = 2;
  ospDeviceSet1i(dev, "logLevel", loglevel);
  ospDeviceCommit(dev);
...
OSPRayRenderer::~OSPRayRenderer(void) {
  ...
  if (context_created && (ospModel != NULL))
    ospRelease(ospModel);
...
void OSPRayRenderer::setup_context(int w, int h) {
  ...
  if (getenv("VMDOSPRAYMAXDEPTH")) {
    ...
    if (maxdepth > 0 && maxdepth <= 20) {
      ...
      printf("OSPRayRenderer) ignoring out-of-range maxdepth: %d...\n", maxdepth);
    }
  } else {
    ospSet1i(ospRenderer, "maxDepth", 20);  
  }

  float scene_epsilon = 5.e-5f;
  if (getenv("VMDOSPRAYSCENEEPSILON") != NULL) {
    scene_epsilon = float(atof(getenv("VMDOSPRAYSCENEEPSILON")));
...
void OSPRayRenderer::framebuffer_config(int fbwidth, int fbheight) {
  if (!context_created)
    return;
...
void OSPRayRenderer::framebuffer_resize(int fbwidth, int fbheight) {
  if (!context_created)
    return;
  ...
  if (buffers_allocated) {
    if (verbose == RT_VERB_DEBUG) 
      printf("OSPRayRenderer) framebuffer_resize(%d x %d)\n", width, height);
    framebuffer_destroy();
  }
...
void OSPRayRenderer::framebuffer_destroy() {
  if (!context_created)
    return;
...
void OSPRayRenderer::render_compile_and_validate(void) {
  ...
  if (camera_projection == OSPRayRenderer::RT_ORTHOGRAPHIC) {
    ...
    ospSet1f(ospCamera, "aspect", width / ((float) height));
    ospSet1f(ospCamera, "fovy", 2.0f*180.0f*(atanf(cam_zoom)/float(VMD_PI)));
  ...
  for (i=0; i<trimesh_v3f_n3f_c3f.num(); i++) {
    if (verbose == RT_VERB_DEBUG)
      printf("OSPRayRenderer) Adding triangle mesh[%d]: %d tris ...\n", 
             i, trimesh_v3f_n3f_c3f[i].numtris);
    ospAddGeometry(ospModel, trimesh_v3f_n3f_c3f[i].geom);
    ospRelease(trimesh_v3f_n3f_c3f[i].geom);
  } 
  ...
  for (i=0; i<spheres_color.num(); i++) {
    ...
    ospRelease(spheres_color[i].geom);
  } 
  ...
  for (i=0; i<cylinders_color.num(); i++) {
    ...
    ospRelease(cylinders_color[i].geom);
  }
...
//printf("OSPRayRenderer) ospRenderFrame(): [%d] ...\n", accum_sample);
        ...
        if (lasterror == 0 /* XXX SUCCESS */) {
          if (frame_ready) {
            double bufnewtime = wkf_timer_timenow(osp_timer);
            ...
            mapbuftotaltime = wkf_timer_timenow(osp_timer) - bufnewtime;
...
void OSPRayRenderer::cylinder_array(Matrix4 *wtrans, float radius,
  ...
  free(ca.cylinders); // ospSetData() is copying presently...
  ca.cylinders = NULL;
  free(ca.colors);
  ca.colors = NULL;
...
void OSPRayRenderer::cylinder_array_color(Matrix4 & wtrans, float rscale,
  ...
  free(cac.cylinders); // ospSetData() is copying presently...
  cac.cylinders = NULL;
  free(cac.colors);
  cac.colors = NULL;
...
void OSPRayRenderer::sphere_array(Matrix4 *wtrans, float rscale,
  ...
  free(sp.xyzr); // ospSetData() is copying presently...
  sp.xyzr = NULL;
  free(sp.colors);
  sp.colors = NULL;
...
void OSPRayRenderer::append_trimesh_v3f_n3f_c3f(osp_trimesh_v3f_n3f_c3f &mesh) {
  mesh.geom  = ospNewGeometry("triangles");
  mesh.verts = ospNewData(mesh.numverts, OSP_FLOAT3, mesh.v, 0);
  mesh.norms = ospNewData(mesh.numverts, OSP_FLOAT3, mesh.n, 0);
  mesh.cols  = ospNewData(mesh.numverts, OSP_FLOAT4, mesh.c, 0);
  mesh.ind   = ospNewData(mesh.numtris, OSP_INT3,   mesh.f, 0);
  ...
  set_material(mesh.geom, mesh.matindex, NULL);
  ...
  free(mesh.v); // ospSetData() is copying presently...
  mesh.v = NULL;
  free(mesh.n);
  mesh.n = NULL;
  free(mesh.c);
...
//  alpha = materialcache[matindex].opacity;
  ...
  append_trimesh_v3f_n3f_c3f(mesh);
}
...
void OSPRayRenderer::trimesh_c4n3v3(Matrix4 & wtrans, int numverts,
  ...
  osp_trimesh_v3f_n3f_c3f mesh;
  memset(&mesh, 0, sizeof(mesh));
  mesh.numtris = numfacets;
  mesh.numverts = 3L * numfacets;
  mesh.matindex = matindex;
