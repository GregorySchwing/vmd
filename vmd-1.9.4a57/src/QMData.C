 ...
 *	$Revision: 1.108 $	$Date: 2020/02/24 17:48:30 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
int QMData::assign_wavef_id(int type, int spin, int exci, char *info,
  ...
  if (!info)
    ...
    strncpy(signa_curts[num_signa_curts].info, info, QMDATA_BUFSIZ-1);
  num_signa_curts++;
  ...
  if (idtag<0 || duplicate) {
    ...
    if (!info)
      ...
      strncpy(wavef_signa[num_wavef_signa].info, info, QMDATA_BUFSIZ-1);
...
/// wavefunction, with orbitals localized using
...
/// 4916-4926.
///
/// The algorithm is described more computationally in:
...
/// 736-740.
///
/// The functions below follow the terminology of the latter paper.
///
