 ...
 *	$Revision: 1.185 $	$Date: 2021/09/23 15:25:55 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
int get_weights_from_attribute(VMDApp *app, AtomSel *sel,
  ...
  for (int i=sel->firstsel; i<=sel->lastsel; i++) {
    weights[i] = sel->on[i] ? static_cast<float>(tmp_data[i]) : 0.0f;
  }
...
static int vmd_measure_avpos(VMDApp *app, int argc, Tcl_Obj * const objv[], Tcl_Interp *interp) {
  ...
  for (i=2; i<argc; i+=2) {
    ...
    if (!strupncmp(argvcur, "first", CMDLEN)) {
      ...
      Tcl_AppendResult(interp, "measure rmsdmat_qcp: invalid syntax, no such keyword: ", argvcur, NULL);
      return TCL_ERROR;
    }
  }
...
static int vmd_measure_rmsdmat_qcp(VMDApp *app, int argc, Tcl_Obj * const objv[], Tcl_Interp *interp) {
  ...
  //If no last frame is given, set it to be the last frame so the framecount is correct.
  if (last < 0)
    last = app->molecule_numframes(sel->molid()) - 1;
...
//////////////////////////////////////////////
// Experimental out-of-core variant 
//
// measure rmsdmat_qcp $sel1 [weight <weights>] start s  end e  step s
static int vmd_measure_rmsdmat_qcp_ooc(VMDApp *app, int argc, Tcl_Obj * const objv[], Tcl_Interp *interp) {
  int first = 0;  // start with first frame by default
  int last = -1;  // finish with last frame by default
  int step = 1;   // use all frames by default
  int i;
  float *weight = NULL;
  int nfiles = 0;
  const char **trjfileset = NULL;

  if (argc < 2) {
    Tcl_WrongNumArgs(interp, 2, objv-1, (char *) "<sel> [weight <weights>]  [first <first>] [last <last>] [step <step>] files [file1] [fileN]");
    return TCL_ERROR;
  }
  AtomSel *sel = tcl_commands_get_sel(interp, Tcl_GetStringFromObj(objv[1],NULL)
);
  ...
  for (i=2; i<argc; i+=2) {
    ...
    if (!strupncmp(argvcur, "first", CMDLEN)) {
      if (Tcl_GetIntFromObj(interp, objv[i+1], &first) != TCL_OK) {
        Tcl_AppendResult(interp, "measure rmsdmat_qcp_ooc: bad first frame value", NULL);
        return TCL_ERROR;
      }
    } else if (!strupncmp(argvcur, "last", CMDLEN)) {
      if (Tcl_GetIntFromObj(interp, objv[i+1], &last) != TCL_OK) {
        Tcl_AppendResult(interp, "measure rmsdmat_qcp_ooc: bad last frame value", NULL);
        return TCL_ERROR;
      }
    } else if (!strupncmp(argvcur, "step", CMDLEN)) {
      if (Tcl_GetIntFromObj(interp, objv[i+1], &step) != TCL_OK) {
        Tcl_AppendResult(interp, "measure rmsdmat_qcp_ooc: bad frame step value", NULL);
        return TCL_ERROR;
      }
    } else if (!strupncmp(argvcur, "files", CMDLEN)) {
      int list_num;
      Tcl_Obj **list_data;
      if (Tcl_ListObjGetElements(interp, objv[i+1], &list_num, &list_data) != TCL_OK) {
        Tcl_AppendResult(interp, "measure rmsdmat_qcp_ooc: bad trajectory file list", NULL);
        return TCL_ERROR;
      }

      int f;
      nfiles = list_num;
      trjfileset = (const char **) calloc(1, nfiles * sizeof(const char *));
      for (f=0; f<nfiles; f++) {
        trjfileset[f] = Tcl_GetStringFromObj(list_data[f], NULL);
        printf("File[%d] '%s'\n", f, trjfileset[f]);
      }
    } else {
      Tcl_AppendResult(interp, "measure rmsdmat_qcp_ooc: invalid syntax, no such keyword: ", argvcur, NULL);
      return TCL_ERROR;
    }
  }
  ...
  // compute the rmsd matrix
  float *rmsdmat = NULL;
  int framecount = 0; // returned after work is done
  int ret_val = measure_rmsdmat_qcp_ooc(app, sel, app->moleculeList, 
                                        nfiles, trjfileset,
                                        sel->selected, weight, 
                                        first, last, step, 
                                        framecount, rmsdmat);
  ...
  if (ret_val < 0) {
    Tcl_AppendResult(interp, "measure rmsdmat_qcp_ooc: ", measure_error(ret_val), NULL);
    return TCL_ERROR;
  }
...
static int vmd_measure_energy(VMDApp *app, int argc, Tcl_Obj * const objv[], Tcl_Interp *interp) {
  ...
  ResizeArray<float> gValues(1024);
  int ret_val;
  ret_val = measure_energy(app->moleculeList, molid, atmid, reqatoms, &gValues, frame, first, last, defmolid, params, geomtype);
  if (ret_val<0) {
    printf("ERROR\n %s\n", measure_error(ret_val));
    Tcl_AppendResult(interp, measure_error(ret_val), NULL);
    return TCL_ERROR;
...
static int vmd_measure_pbc2onc_transform(VMDApp *app, int argc, Tcl_Obj * const objv[], Tcl_Interp *interp) {
  ...
  if (argc>3) {
    for (i=2; i<argc; i+=2) {
      char *argvcur = Tcl_GetStringFromObj(objv[i],NULL);
      if (!strupncmp(argvcur, "molid", CMDLEN)) {
        // XXX lame test formulation should be corrected
...
static int vmd_measure_pbc_neighbors(VMDApp *app, int argc, Tcl_Obj * const objv[], Tcl_Interp *interp) {
  ...
  int ret_val = measure_pbc_neighbors(app->moleculeList, sel, molid, frame, &alignment, center, cutoff, boxminmax, &extcoord_array, &indexmap_array);
  delete [] boxminmax;
...
int vmd_measure_volinterior(VMDApp *app, int argc, Tcl_Obj * const objv[], Tcl_Interp *interp) {
  ...
  if (argc < 3) {
    ...
    Tcl_SetResult(interp, (char *) "usage: volinterior "
                  ...
                  "-probmap (compute and load probability map)\n"
                  "   --> -count_pmap (return percentile-based voxel counts)\n"
                  "-discretize [float cutoff] (creates discrete map with cutoff probability)\n"
  ...
  for (int i=2; i < argc; i++) {
    ...
    if (!strcmp(opt, "-probmap")) {
      loadpmap=1;
      recognized=1;
    }
    if (!strcmp(opt, "-checkrays")) {
      print_raydirs=1;
      recognized=1;
    }
    if (!strcmp(opt, "-count_pmap")) {
      count_pmap=1;
      recognized=1;
    }
    if (!strcmp(opt, "-discretize")) {
      double tcutoff;
      if (i==argc-1) {
        Tcl_AppendResult(interp, "No cutoff probability given for -discretize flag",NULL);
        return TCL_ERROR;
      } else if (Tcl_GetDoubleFromObj(interp, objv[i+1], &tcutoff)!=TCL_OK) {
        Tcl_AppendResult(interp, "\n Cutoff probability for -discretize flag incorrectly specified.",NULL);
        return TCL_ERROR;
      }
      proCutoff=(float)tcutoff;
      recognized=1;
      process=1;
    }
    if (!strcmp(opt, "-vol")) {
...
//    }
    ...
    if (recognized==0) {
      // XXX what case is this supposed to handle? Dangling number parms?
      if (isdigit(opt[0]) || isfloat(opt)) { 
        continue;
      } else {
  ...
  if ((process||count_pmap) && !loadpmap) {
  ...
  Molecule *pmolOverwrite=NULL;
  VolumetricData *pmapOverwrite=NULL;
  if (overwrite==1) {
    if (loadpmap!=1) {
      ...
      pmolOverwrite=mlist->mol_from_id(molid);
      pmapOverwrite=pmolOverwrite->modify_volume_data(volidOverwrite);
      if (pmapOverwrite==NULL) {
  ...
  VolumetricData *pmapT = NULL;
  VolumetricData *procmap = NULL;
  VolumetricData *pmap = NULL;  
  ...
  radscale= 0.2f * float(resolution);
  if (gspacing == 0) {
  ...
  if (molid != -1 && volid != -1 ) {
    ...
    int cuda_err=-1;
#if defined(VMDCUDA)
    cuda_err = vmd_cuda_calc_density(sel, app->moleculeList, quality,
                                     radscale, gspacing, &volmapA, 
                                     NULL, NULL, verbose);
#endif
    if (cuda_err == -1 ) {
      if (verbose) printf("Using CPU version of the code ... \n");
      volmapA = qs->calc_density_map(sel, mymol, framepos, radii,
                                     quality, radscale, float(gspacing));
    }
  ...
  Nout += RaycastGrid(volmapA,target,isovalue,rayDir);
  ...
  long nVoxIso=markIsoGrid(volmapA, target,isovalue);
  if (verbose) printf("Casting rays ...\n");
  float rayDir[3] = {0.1f,0.1f,1.0f};
  Nout += volin_threaded(volmapA,target,isovalue,rayDir);
  ...
  float *poisson_set = new float[(nrays+1)*2];
  int numcandidates=40;
  int converged=poisson_sample_on_sphere(poisson_set,nrays,numcandidates,verbose);
  if (converged==0) {
    if (verbose) {
      printf("Poisson disk sampler failed for measure volinterior!\n");
      printf("... continuing with standard ray direction generator.\n");
    }
  }

  if (print_raydirs && converged)
    print_xyz(poisson_set,nrays);
  ...
  // XXX Use of sincosf() now in place. It would be much more work-efficient
  //     to build up batches of rays and pass them into the multithreaded
  //     work routines, as launching the worker threads has a non-trivial
  //     cost.  It would be better if they plow through many/all rays before
  //     returning, since the control loops here aren't checking for any
  //     conditions to be met other than that we've processed all of the rays.
  ...
  for (int ray=0; ray < nrays; ray++) { 
    ...
    if (converged==1) {
      sincosf(poisson_set[ray*2+0],&sine1,&cosine1);
      sincosf(poisson_set[ray*2+1],&sine2,&cosine2);
      rayDir[0]=cosine1*sine2;
      rayDir[1]=sine1*sine2;
      rayDir[2]=cosine2;
    } else {
      sincosf(phi,&sine1,&cosine1);
      rayDir[0]=R*cosine1;
      rayDir[1]=R*sine1;
      rayDir[2]=z;
    }

    // XXX rather than launching threads per ray, we should be launching
    //     the worker threads and processing _all_ of the rays...
    //     That would also lead toward an implementation that could
    //     eventually be migrated to the GPU for higher performance.
    if (loadpmap == 1) {
      Nout += volin_threaded_prob(volmapA, target, pmapT, isovalue, rayDir);
    } else {
      Nout += volin_threaded(volmapA, target, isovalue, rayDir);
    }
  ...
  Nout=countIsoGrids(target,EXTERIORVOXEL);
  long nIn=countIsoGrids(target,INTERIORVOXEL);
  ...
  if (overwrite != 1 ) {
    if (loadpmap==1) {
      if (verbose) printf("Normalizing probability map ...\n");
      pmap = normalize_pmap(pmapT, nrays);
      app->molecule_add_volumetric(molid, pmap->name, pmap->origin,
                                   pmap->xaxis, pmap->yaxis, pmap->zaxis,
                                   pmap->xsize, pmap->ysize, pmap->zsize, 
                                   pmap->data);
      pmap->compute_volume_gradient();
    } else {
      app->molecule_add_volumetric(molid, target->name, target->origin, 
                                   target->xaxis, target->yaxis, target->zaxis,
                                   target->xsize, target->ysize, target->zsize,
                                   target->data);
    ...
    if (loadpmap==1) {
      ...
      pmapOverwrite->name=pmap->name;
      pmapOverwrite->xsize=pmap->xsize;
      pmapOverwrite->ysize=pmap->ysize;
      pmapOverwrite->zsize=pmap->zsize;
      pmapOverwrite->data=pmap->data;
      pmap->data=NULL;
      pmapOverwrite->compute_volume_gradient();
      if (verbose) printf("Done!\n");
    } else {
      if (verbose) printf("Overwriting volume ... \n");
  ...
  if ((loadpmap&&process)&&pmap->data!=NULL) {
    //Tcl_AppendResult(interp,"Processing probability map with cutoff: ",proCutoff,"...",NULL);
    procmap=process_pmap(pmap, proCutoff);
    app->molecule_add_volumetric(molid, procmap->name, procmap->origin,
      procmap->xaxis, procmap->yaxis, procmap->zaxis,
      procmap->xsize, procmap->ysize, procmap->zsize, procmap->data);
    procmap->compute_volume_gradient();
    procmap->data=NULL;
    if (verbose) printf("Loaded processed map\n");
  } else if ((loadpmap&&process)&&pmapOverwrite->data!=NULL) {
     procmap=process_pmap(pmapOverwrite, proCutoff);
     app->molecule_add_volumetric(molid, procmap->name, procmap->origin,
       procmap->xaxis, procmap->yaxis, procmap->zaxis,
       procmap->xsize, procmap->ysize, procmap->zsize, procmap->data);
     procmap->compute_volume_gradient();
     procmap->data=NULL;
    if (verbose) printf("Loaded processed map (they don't overwrite).\n");
  }

  if (count_pmap==1 && pmap->data!=NULL) {
    if (loadpmap!=1) {
      printf("-count_pmap flag specified without -probmap!\n");
      return TCL_ERROR;
    } else {
      long Pvol=0;
      float currProb=0.0f;
      Tcl_Obj *analysis=Tcl_NewListObj(0,NULL);
      for (int c=0; c<10; c++) {
        currProb+=0.1f;
        if (c==9) currProb=((float)nrays-((float)nrays*0.01f))/(float)nrays;
        Pvol=vol_probability(pmap, currProb, float(gspacing));
        Tcl_ListObjAppendElement(interp, analysis, Tcl_NewLongObj(Pvol));
      }
      Tcl_SetObjResult(interp,analysis);
      if (molid != -1 && volid != -1 ) {
        target->data = NULL; // prevent destruction of density array;
        pmap->data = NULL;
      } else {
  ...
  } else if (count_pmap==1 && overwrite==1 && pmapOverwrite->data!=NULL) {
    long Pvol=0;
    float currProb=0.0f;
    Tcl_Obj *analysis=Tcl_NewListObj(0,NULL);
    for (int c=0; c<10; c++) {
      currProb+=0.1f;
      if (c==9) currProb=((float)nrays-((float)nrays*0.01f))/(float)nrays;
      Pvol=vol_probability(pmapOverwrite, currProb, float(gspacing));
      Tcl_ListObjAppendElement(interp, analysis, Tcl_NewLongObj(Pvol));
    }
    Tcl_SetObjResult(interp,analysis);
    if (molid != -1 && volid != -1 ) {
...
int obj_measure(ClientData cd, Tcl_Interp *interp, int argc,
  ...
  if (argc < 2) {
    Tcl_SetResult(interp, 
      ...
      "  volinterior <sel> [options]\n"
      "     -- type 'measure volinterior' for usage\n",
      TCL_STATIC);
    return TCL_ERROR;
  }
  ...
  else if (!strupncmp(argv1, "rmsdmat_qcp_ooc", CMDLEN))
    return vmd_measure_rmsdmat_qcp_ooc(app, argc-1, objv+1, interp);
#endif
