 ...
 *	$Revision: 1.31 $	$Date: 2021/02/11 18:51:41 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
// fctn prototypes for AVX, AVX2, AVX-512 runtime dispatch
...
#if (defined(VMDUSESSE) && defined(__SSE2__)) || (defined(VMDUSEVSX) && defined(__VSX__)) || (defined(VMDUSEAVX) && defined(__AVX__)) || (defined(VMDUSENEON) && defined(__ARM_NEON__))
...
int analyze_selection_aligned_dispatch(wkf_cpu_caps_t *cpucaps,
  ...
  if (cpucaps != NULL) {
//printf("analyze_selection_aligned_dispatch()\n");

    // Intel x86
#if (defined(__i386__) || defined(__x86_64__) || defined(_M_IX86) || defined(_M_AMD64))
    if (cpucaps->flags & CPU_AVX2) {
//printf("analyze_selection_aligned_avx2()\n");
      return analyze_selection_aligned_avx2(n, on, firstsel, lastsel, selected);
    }

    if (cpucaps->flags & CPU_AVX) {
//printf("analyze_selection_aligned_avx()\n");
      return analyze_selection_aligned_avx(n, on, firstsel, lastsel, selected);
    }
#endif
...
void minmaxmean_1fv_aligned(const float *f, ptrdiff_t n, 
  ...
  *fmean = mean1 / n; 
  ...
  // Clang bugs checked with compiler explorer:  http://godbolt.org/
  __m128d mean2d = _mm_cvtps_pd(_mm_set_ps1(0.0f)); // XXX Clang workaround
#else
  //  __m128d mean2d = _mm_set_pd1(0.0); // XXX Clang v < 5.x misses this
  __m128d mean2d = _mm_set1_pd(0.0); // XXX Clang v < 5.x misses this
#endif
...
void minmax_3fv_aligned(const float *f, const ptrdiff_t n3, 
...
// Compute min/max values for a 16-byte-aligned array of float3s
// input value n3 is the number of 3-element vectors to process
int minmax_selected_3fv_aligned(const float *f, const int *on, 
                                const ptrdiff_t n3, 
  ...
  ptrdiff_t i=firstsel;
  minx=maxx=f[i*3L  ];
  miny=maxy=f[i*3L+1];
  minz=maxz=f[i*3L+2];

  ptrdiff_t end=lastsel+1;
...
//        i, &on[i], ind3, &f[ind3]);
  ...
  for (; i<(end-3); i+=4) {
    ...
    // compute mins and maxes
    xmin4 = _mm_sel_ps(xmin4, _mm_min_ps(xmin4, x), _mm_castsi128_ps(mask));
    xmax4 = _mm_sel_ps(xmax4, _mm_max_ps(xmax4, x), _mm_castsi128_ps(mask)); 
    ymin4 = _mm_sel_ps(ymin4, _mm_min_ps(ymin4, y), _mm_castsi128_ps(mask)); 
    ymax4 = _mm_sel_ps(ymax4, _mm_max_ps(ymax4, y), _mm_castsi128_ps(mask)); 
    zmin4 = _mm_sel_ps(zmin4, _mm_min_ps(zmin4, z), _mm_castsi128_ps(mask)); 
    zmax4 = _mm_sel_ps(zmax4, _mm_max_ps(zmax4, z), _mm_castsi128_ps(mask)); 
  }
