 ...
 *      $Revision: 1.181 $      $Date: 2022/01/21 07:58:29 $
 *
 ***************************************************************************
 * DESCRIPTION:
 * 
...
static int atomsel_set_phi(void *v, int num, double *data, int *flgs) {
  ...
  for (i=0; i<atom_sel_mol->fragList.num(); i++) {
    ...
    for (int ires = 1; ires < nfragres; ires++) {
      ...
      AtomSel *nterm = new AtomSel(atom_sel_mol->app,
                                   table, atom_sel_mol->id());
      char buf[100];
      sprintf(buf, 
...
static int atomsel_set_psi(void *v, int num, double *data, int *flgs) {
  ...
  for (int i=0; i<atom_sel_mol->fragList.num(); i++) {
    ...
    for (int ires = nfragres-2; ires >= 0; ires--) {
      ...
      AtomSel *cterm = new AtomSel(atom_sel_mol->app,
                                   table, atom_sel_mol->id());
      char buf[100];
      sprintf(buf,
				    ...
				    int num, int *flgs)
{
  ...
  for (int word=0; word<argc; word++) {
    ...
    // and do the search
    AtomSel *atomSel = new AtomSel(atom_sel_mol->app,
                                   table, atom_sel_mol->id());
...
int atomsel_custom_singleword(void *v, int num, int *flgs) {
  ...
  AtomSel *atomSel = new AtomSel(mol->app, table, mol->id());
...
static int atomsel_gridindex_array(void *v, int num, double *data, int *flgs, int array_index) {
  ...
  for (i=0; i<num; i++) {					      
    if (flgs[i]) 							      
      data[i] = double(voldata->voxel_index_from_coord(ts->pos[3L*i], ts->pos[3L*i+1], ts->pos[3L*i+2]));
  }									      
  return 1;								     
}
...
AtomSel::AtomSel(VMDApp *vmdapp, SymbolTable *st, int id)
: ID(id) {
...
int AtomSel::change(const char *newcmd, DrawMolecule *mol) {
  ...
  if (analyze_selection_aligned_dispatch(app->cpucaps, num_atoms, on, &firstsel, &lastsel, &selected))
    return ret_code;

#if 0
  // (re)allocate and populate selection group acceleration data structure
  num_atoms256 = num_atoms >> 8; // natoms/256
  if (on256) delete [] on256;
  on256 = new unsigned int[num_atoms256];

  int firstblk = firstsel >> 8;
  int lastblk = lastsel >> 8;
  for (int blk=firstblk; blk<=lastblk; blk++) {
    // loop and test only the atoms in block[blk]
    int firstatom = blk << 8;
    int lastatom = ((blk+1) << 8) - 1;
    int blkcnt = 0;
    int i=0;

    on256 = 0; // no atoms selected initially
    for (i=firstatom; i<=lastatom; i++) {
      if (on[i]) {
        // set selection flag non-zero indicating that something within
        // this 256-atom block is selected...
        on256[blk] = 1 << 24; // reserve lower 3 bytes for indices/counter

        break; // once we've found a selected atom, we can test next blk
      }
    }

#if 0
    // XXX a next-gen scheme could break up the 32-bit integer into
    // four 8-bit quantities that indicate the first-selected 
    // index (relative to the start of the block), the last-selected
    // index (relative to the start of the block), and the total number
    // of selected atoms in the block.  Since we work with 256-atom
    // blocks, each unsigned sub-byte can fully index/count the 
    // contents of the block, allowing block-level optimizations 
    // identical in design to what we have done previously at the
    // level of entire selection arrays.
    on[i] |= (i & 0xFF) << 8; // first selected in block 
    for (; i<=lastatom; i++) {
      if (on[i]) {
        blkcnt++; // count selected atoms...
      }
    }
    on[i] |= (i & 0xFF) << 16; // last selected in block 
    on[i] |= (blkcnt & 0xFF);  // count of selected atoms in block
#endif
