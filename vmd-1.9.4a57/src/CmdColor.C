 ...
 *      $Revision: 1.48 $      $Date: 2020/12/24 04:21:51 $
 *
 ***************************************************************************
 * DESCRIPTION:
 * 
...
CmdColorScaleSettings::CmdColorScaleSettings(float newmid, float newmin, 
                                             float newmax, int newrev,
                                             int newposterize)
: Command(Command::COLOR_SCALE_SETTINGS), mid(newmid), min(newmin), max(newmax), rev(newrev), posterize(newposterize)
{}
