 ...
 *      $Revision: 1.20 $       $Date: 2020/07/23 03:27:52 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
public:
  AtomSelThr(VMDApp *vmdapp, AtomSel *osel, wkf_mutex_t *olock)
      : AtomSel(vmdapp, NULL, osel->molid()),
        sel(osel), lock(olock) {
    if (sel) {
...
private:
//  AtomSelThr() : AtomSel(NULL, NULL, -1) {};
  AtomSelThr& operator=(const AtomSelThr &) { return *this; };
//  AtomSelThr(AtomSelThr &) : AtomSel(NULL, NULL, -1) {};
                              ...
                              const double cutoff, float *weights)
{
  ...
  for (i=0; i<numprocs; ++i) {
    ...
    parms[i].sel = new AtomSelThr(mymol->app, sel, atomsel_lock);
    parms[i].mol = mymol;
