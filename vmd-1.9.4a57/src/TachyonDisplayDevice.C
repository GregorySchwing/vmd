...
*      $Revision: 1.135 $        $Date: 2022/03/21 01:54:41 $
*
***************************************************************************/
/**
 *  \file TachyonDisplayDevice.C
...
#if 1
    // Tachyon 0.99.2 or later adds AO max occlusion distance parameter
    if (getenv("VMDTACHYONAOMAXDIST")) {
      float tmp = float(atof(getenv("VMDTACHYONAOMAXDIST")));
      printf("LibTachyonDisplayDevice) setting AO maxdist: %f\n", tmp);
      fprintf(outfile, "    MaxDist %g\n", tmp);
    }
#endif
...
void TachyonDisplayDevice::write_colormaterial(float *rgb, int /* material */) {
  ...
  if (!involtex) {
    ...
    // transform the u/v/w texture coordinate axes from molecule
