 ...
 *      $Revision: 1.125 $      $Date: 2022/04/23 06:03:56 $
 *
 ***************************************************************************/
/**
 * \file CUDAOrbital.cu
 ...
 *
 *  "High Performance Computation and Interactive Display of Molecular Orbitals
...

//
// This is an Ampere A6000-specific kernel designed to rely entirely on
// L1 cache for access to various coefficient arrays.  It would perform
// terribly on older devices so it should not be used on such hardware.
//
__global__ static void cuorbitalcachedglobmem_packed(int numatoms,
                          const float * RESTRICT wave_f,
                          const float2 * RESTRICT basis_array,
                          const flint * RESTRICT atominfo,
                          const int2 * RESTRICT shellinfo,
                          float voxelsize,
                          float originx,
                          float originy,
                          float grid_z,
  ...
  for (at = 0; at < numatoms; at++) {
    ...
    int prim_counter = atominfo[atidx + 3].i >> 1;
    int maxshell     = atominfo[atidx + 4].i;
    ...
    // loop over the shells belonging to this atom (or basis function)
    for (shell=0; shell < maxshell; shell++) {
      int2 tmpshellinfo = shellinfo[shell_counter];
      int maxprim    = tmpshellinfo.x;
      int shell_type = tmpshellinfo.y;

      float contracted_gto = 0.0f;
      for (prim=prim_counter; prim < maxprim;  prim++) {
        float2 tmpbasis = basis_array[prim];
        float exponent       = tmpbasis.x;
        float contract_coeff = tmpbasis.y;
...
// terribly on older devices so it should not be used on such hardware.
//
__global__ static void cuorbitalcachedglobmem_packed_2x(int numatoms,
                          const float * RESTRICT wave_f,
                          const float2 * RESTRICT basis_array,
                          const flint * RESTRICT atominfo,
                          const int2 * RESTRICT shellinfo,
                          ...
                          float grid_z,
                          int density,
                          float2 *  RESTRICT orbitalgrid) {
  unsigned int xindex  = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int yindex  = blockIdx.y * blockDim.y + threadIdx.y;

  // outaddr is for a float2, so no unroll factor here
  unsigned int outaddr = gridDim.x * blockDim.x * yindex + xindex;
  float2 grid_x = make_float2(originx) +
                    voxelsize * make_float2(UNROLLX * xindex,
                                            UNROLLX * xindex + 1);
  float grid_y = originy + voxelsize * yindex;
  ...
  for (at = 0; at < numatoms; at++) {
    // calculate distance between grid point and center of atom
    int atidx = at << 4;
    float2 xdist = (grid_x - atominfo[atidx + 0].f)*ANGS_TO_BOHR;
    float ydist = (grid_y - atominfo[atidx + 1].f)*ANGS_TO_BOHR;
    float zdist = (grid_z - atominfo[atidx + 2].f)*ANGS_TO_BOHR;
    ...
    float2 xdist2 = xdist*xdist;
    float ydist2 = ydist*ydist;
    float zdist2 = zdist*zdist;

    float2 dist2 = xdist2 + make_float2(ydist2 + zdist2);
    ...
    for (shell=0; shell < maxshell; shell++) {
      ...
      float2 contracted_gto = make_float2(0.0f);
      for (prim=prim_counter; prim < maxprim;  prim++) {
        float2 tmpbasis = basis_array[prim];
        float exponent       = tmpbasis.x;
        float contract_coeff = tmpbasis.y;
        ...
        // we can use exp2f() rather than exp(), giving us full
        // precision, but with the speed of __expf()
        float2 negexpdist2 = -exponent*dist2;
        float2 expval;
        expval.x = exp2f(negexpdist2.x);
        expval.y = exp2f(negexpdist2.y);
        contracted_gto += contract_coeff * expval;
      }

      /* multiply with the appropriate wavefunction coefficient */
      float2 tmpshell = make_float2(0.0f);
      switch (shell_type) {
        case S_SHELL:
          value += wave_f[ifunc++] * contracted_gto;
          break;
  ...
  if (density) {
    float2 tmpout;
    tmpout.x = copysignf(value.x*value.x, value.x);
    tmpout.y = copysignf(value.y*value.y, value.y);
    orbitalgrid[outaddr] = tmpout;
  } else {
    orbitalgrid[outaddr] = value;
  }
}
...
// terribly on older devices so it should not be used on such hardware.
//
__global__ static void cuorbitalcachedglobmem_packed_4x(int numatoms,
                          const float * RESTRICT wave_f,
                          const float2 * RESTRICT basis_array,
                          const flint * RESTRICT atominfo,
                          const int2 * RESTRICT shellinfo,
                          ...
                          float grid_z,
                          int density,
                          float4 *  RESTRICT orbitalgrid) {
  unsigned int xindex  = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int yindex  = blockIdx.y * blockDim.y + threadIdx.y;

  // outaddr is for a float4, so no unroll factor here
  unsigned int outaddr = gridDim.x * blockDim.x * yindex + xindex;
  float4 grid_x = make_float4(originx) +
                    voxelsize * make_float4(UNROLLX * xindex,
                                            UNROLLX * xindex + 1,
                                            UNROLLX * xindex + 2,
                                            UNROLLX * xindex + 3);
  float grid_y = originy + voxelsize * yindex;
  ...
  for (at = 0; at < numatoms; at++) {
    // calculate distance between grid point and center of atom
    int atidx = at << 4;
    float4 xdist = (grid_x - atominfo[atidx + 0].f)*ANGS_TO_BOHR;
    float ydist = (grid_y - atominfo[atidx + 1].f)*ANGS_TO_BOHR;
    float zdist = (grid_z - atominfo[atidx + 2].f)*ANGS_TO_BOHR;
    ...
    float4 dist2 = xdist2 + make_float4(ydist2 + zdist2);
    ...
    for (shell=0; shell < maxshell; shell++) {
      ...
      float4 contracted_gto = make_float4(0.0f);
      for (prim=prim_counter; prim < maxprim;  prim++) {
        float2 tmpbasis = basis_array[prim];
        float exponent       = tmpbasis.x;
        float contract_coeff = tmpbasis.y;
        ...
        // we can use exp2f() rather than exp(), giving us full
        // precision, but with the speed of __expf()
        float4 negexpdist2 = -exponent*dist2;
        float4 expval;
        expval.x = exp2f(negexpdist2.x);
        expval.y = exp2f(negexpdist2.y);
        expval.z = exp2f(negexpdist2.z);
        expval.w = exp2f(negexpdist2.w);
        contracted_gto += contract_coeff * expval;
      }

      /* multiply with the appropriate wavefunction coefficient */
      float4 tmpshell = make_float4(0.0f);
      switch (shell_type) {
        case S_SHELL:
          value += wave_f[ifunc++] * contracted_gto;
          break;
  ...
  if (density) {
    ...
    tmpout.x = copysignf(value.x*value.x, value.x);
    tmpout.y = copysignf(value.y*value.y, value.y);
    tmpout.z = copysignf(value.z*value.z, value.z);
    tmpout.w = copysignf(value.w*value.w, value.w);
    orbitalgrid[outaddr] = tmpout;
  } else {
    orbitalgrid[outaddr] = value;
  }
...
static void * cudaorbitalthread(void *voidparms) {
  ...
  // default tile size to use in absence of other info
  int tilesize=4; // GTX 280, Tesla C1060 starting point tile size
  ...
  if ((cudaGetDevice(&dev) == cudaSuccess) &&
    ...
    if (deviceProp.major >= 7) 
      hwpreferscachekernel=7; // Volta hardware prefers the cache kernel

    // allow overrides for testing purposes
    if (getenv("VMDFORCEMOPACKED") != NULL) {
      if (deviceProp.major >= 8) 
        hwpreferscachekernel=8; // Ampere hardware prefers the cache kernel
    }
  }
  ...
  float log2e = log2f(2.718281828f);
...
#if 1
    //
    // Packed shellinfo array for latest Volta/Ampere GPUs
    //
    padsz = computepaddedsize(2 * parms->num_shells, MEMCOALESCE);
    h_shellinfo = (int *) calloc(1, padsz * sizeof(int));
    cudaMalloc((void**)&d_shellinfo_packed, padsz * sizeof(int));
    for (int ll=0; ll<parms->num_shells; ll++) {
      h_shellinfo[ll*2    ] = parms->num_prim_per_shell[ll];
      h_shellinfo[ll*2 + 1] = parms->shell_types[ll];
    }
    cudaMemcpy(d_shellinfo_packed, h_shellinfo, padsz * sizeof(int), cudaMemcpyHostToDevice);
    free(h_shellinfo);
#endif
  ...
  while (wkf_threadpool_next_tile(voidparms, tilesize, &tile) != WKF_SCHED_DONE) {
    for (int k=tile.start; k<tile.end; k++) {
      ...
      if (usefastconstkernel) {
        ...
        if (usecachekernel) {
          // Ampere hardware prefers the "packed" cache kernel
          if (hwpreferscachekernel >= 8) {
#if 1
            cuorbitalcachedglobmem_packed_4x<<<Gsz, Bsz, 0>>>(parms->numatoms,
                                 d_wave_f, (float2 *) d_basis_array,
                                 d_atominfo, (int2 *) d_shellinfo_packed,
                                 parms->voxelsize,
                                 origin[0], origin[1], origin[2],
                                 parms->density, (float4 *) d_orbitalgrid);
#elif 1
            cuorbitalcachedglobmem_packed_2x<<<Gsz, Bsz, 0>>>(parms->numatoms,
                                 d_wave_f, (float2 *) d_basis_array,
                                 d_atominfo, (int2 *) d_shellinfo_packed,
                                 parms->voxelsize,
                                 origin[0], origin[1], origin[2],
                                 parms->density, (float2 *) d_orbitalgrid);
#else
            cuorbitalcachedglobmem_packed<<<Gsz, Bsz, 0>>>(parms->numatoms,
                                 d_wave_f, (float2 *) d_basis_array,
                                 d_atominfo, (int2 *) d_shellinfo_packed,
                                 parms->voxelsize,
                                 origin[0], origin[1], origin[2],
                                 parms->density, d_orbitalgrid);
#endif
          } else {
            ...
            cuorbitalcachedglobmem<<<Gsz, Bsz, 0>>>(parms->numatoms, 
                                 d_wave_f, d_basis_array,
                                 d_atominfo, d_shellinfo,
                                 parms->voxelsize,
                                 origin[0], origin[1], origin[2],
                                 parms->density, d_orbitalgrid);
          }
      ...
      int planesz = numvoxels[0] * numvoxels[1];
      for (int y=0; y<numvoxels[1]; y++) {
        long orbaddr = k*planesz + y*numvoxels[0];
