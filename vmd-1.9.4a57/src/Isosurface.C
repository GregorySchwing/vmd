...
int IsoSurface::compute(VolumetricData *data, float isovalue, int step) {
  int x, y, z; 
  int tricount=0;
...
//
int IsoSurface::DoGridPosNorms(float isovalue, int step) {
  GRIDCELL gc;
  TRIANGLE tris[5];
  long tricount=0;
  long globtricount=0;
  ...
  long psz = vol->xsize * vol->ysize;
  long rsz = vol->xsize;
  long rowstep = rsz*step;
  long planestep = psz*step;
  ...
  for (z=0; z<(vol->zsize - step); z+=step) {
    for (y=0; y<(vol->ysize - step); y+=step) {
      for (x=0; x<(vol->xsize - step); x+=step) {
        ...
        // XXX this loop should avoid per-triangle append() operations
        //     in favor of directly writing to the output buffer, by 
        //     first calling the extend() method optimistically, and
        //     then truncating the output size to the actual triangle
        //     count that was emitted.  That would get the ResizeArray
        //     append/check methods entirely out of the innermost loops.
        int i;
        for (i=0; i<tricount; i++) {
          // XXX we need to truncate buffers larger than 32-bit ints since
          //     OpenGL etc won't like such large indices
...
int IsoSurface::DoCellGeneral(int x, int y, int z, float isovalue, int step) {
  GRIDCELL gc;
  int tricount;
  TRIANGLE tris[5];
  ...
  // precompute plane/row sizes to eliminate indirection, promote int types
  long psz = vol->xsize * vol->ysize;
  long rsz = vol->xsize;
  long addr = z*psz + y*rsz + x;
  long rowstep = rsz*step;
...
/// XXX This should be eliminated in favor of an implicit
///     per-vertex color derived from a whole-mesh color.
///     We should eliminate the per-vertex color storage for this case,
///     yielding a nice reduction in PCIe draw bandwidth and memory footprint.
...
int IsoSurface::set_color_voltex_rgb3fv(const float *voltex) {
  ...
  for (i=0; i<numverts; i++) {
    ...
    long caddrp1 = (x < xs) ? caddr+3 : caddr;
