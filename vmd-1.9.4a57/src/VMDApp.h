 ...
 *      $Revision: 1.257 $      $Date: 2020/12/24 04:21:51 $
 *
 ***************************************************************************/
...
public:
  ...
  wkf_cpu_caps_t *cpucaps;      ///< CPUID, SIMD vector insn capability bits

  wkf_threadpool_t *thrpool;    ///< CPU thread pool for low-latency calcs
  ...
  int menu_id(const char *name); ///< return index of named menu, or -1 on fail
  ...
  int colorscale_params(float *midpoint, float *min, float *max, 
                        int *rev, int *posterize); 
  
  /// Change the settings for the color scale.
  int colorscale_setparams(float midpoint, float min, float max, 
                           int rev, int posterize);
