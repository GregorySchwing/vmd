 ...
 *      $Revision: 1.587 $      $Date: 2022/04/09 21:15:13 $
 *
 ***************************************************************************/
...
extern "C" void * affinitize_threads(void *voidparms) {
  ...
  int verbose = (getenv("VMDCPUAFFINITYVERBOSE") != NULL);
...
//  setthreadaffinity=0;
  if (tid == 0 && verbose) {
    printf("Host physcpus: %d  numthreads: %d\n", physcpus, numthreads);
  }
#endif
  ...
  if ((physcpus > 0) && setthreadaffinity) {
    ...
    // debug affinity behavior on P9 w/ IBM's jsrun tool
...
#elif defined(ARCH_SUMMIT) || defined(ARCH_OPENPOWER)
    // on POWER8 CPU SMT "cores" are numbered consecutively, so to spread out
    // threads across all physical cores, threads are assigned to distinct 
    // physical CPU cores by multiplying thread ID * SMT depth, modulo the 
    // total SMT core count.
...
int VMDApp::VMDinit(int argc, char **argv, const char *displaytype, 
  ...
  cpucaps = (wkf_cpu_caps_t *) calloc(1, sizeof(wkf_cpu_caps_t));
#endif
...
#if defined(VMDMPI)
  if (mpienabled) {
    vmd_mpi_nodeinfo(&noderank, &nodecount);
  }
#endif
  ...
  if (!mpienabled) {
    msgInfo << "Multithreading available, " << vmdnumcpus
            << ((vmdnumcpus > 1) ? " CPUs" : " CPU") 
#if defined(VMDCPUDISPATCH)
            << ", ISA dispatch enabled."
#else
            << "."
#endif
            << sendmsg;
  ...
  if (!wkf_cpu_capability_flags((wkf_cpu_caps_t *) cpucaps) &&
      !mpienabled) {
    ...
    if (cpucaps->flags & CPU_HYPERVISOR) {
      msgInfo << sendmsg;
      msgInfo << "  Detected VM or hypervisor execution environment";
    }
#endif

#if (defined(ARCH_LINUXARM64) || defined(__ARM_ARCH_ISA_A64) || defined(__ARM_NEON))
...
#if defined(VMDCPUDISPATCH) && defined(__ARM_ARCH_ISA_A64) && !defined(ARCH_MACOSXARM64)
    if (cpucaps->flags & CPU_ARM64_SVE) {
      int vs32 = arm_sve_vecsize_32bits();
      int vs64 = arm_sve_vecsize_64bits();
      if (vs32 > 0 && vs64 > 0) {
        msgInfo << sendmsg;
        msgInfo << "  ARM64 SVE vector lengths  32-bit: " 
                << vs32 << ",  64-bit: " << vs64;
      }
    }
#endif
  ...
  if (mpienabled) {
    ...
    vmd_mpi_nodescan(NULL, NULL, nodename, sizeof(nodename), 
...
#if defined(ARCH_MACOSXARM64) || defined(ARCH_MACOSXX86_64)
  ...
  atomSelParser = new SymbolTable(this);
...
int VMDApp::VMDupdate(int check_for_events) {
  ...
  if (check_for_events) {
    ...
    if (display->supports_gui()) {
      ...
      Fl::flush();
#elif (defined(ARCH_MACOSXARM64) || defined(ARCH_MACOSXX86) || defined(ARCH_MACOSXX86_64)) && defined(VMDTCL)
...
int VMDApp::colorscale_params(float *mid, float *min, float *max, 
                              int *rev, int *posterize) {
  scene->colorscale_params(mid, min, max, rev, posterize);
  return 1;
}
...
const char *VMDApp::colorscale_method_menuname(int n) {
  if (n < 0 || n >= scene->num_colorscale_methods()) return NULL;
  return scene->colorscale_method_menuname(n);
}
...
int VMDApp::colorscale_setparams(float mid, float min, float max, 
                                 int rev, int posterize) {
  scene->set_colorscale_params(min, mid, max, rev, posterize);
  commandQueue->runcommand(new CmdColorScaleSettings(mid, min, max, rev, posterize));
  return 1;
}
...
int VMDApp::molecule_load(int molid, const char *filename, 
  ...
  if (plugin->can_read_pagealigned_timesteps()) {
    ...
    int fslocality = vmd_fstype_locality(filename);

    if (fslocality == VMD_FS_IS_REMOTE) {
      msgInfo << "Kernel-bypass direct-I/O disabled for remote filesystem" 
              << sendmsg;
    }

#if vmdplugin_ABIVERSION > 17
    // If we don't query the page alignment size, the molfile plugin
  ...
  if (plugin->can_read_structure()) {
    if (!newmol->has_structure()) {
      ...
      // In this case, it is not acceptable to get MOLFILE_NUMATOMS_UNKNOWN,
      // since we must have an atom count to initialize structure information.
      if (plugin->natoms() > 0) {
    ...
    // This implies natoms != MOLFILE_NUMATOMS_UNKNOWN
    if (plugin->natoms() > 0) {
  ...
  //   or the trajectory plugin returns MOLFILE_NUMATOMS_UNKNOWN, indicating
  //   that it has to be paired with a struture file containing a known
  //   atom count.  This is needed by AMBER CRD files, among others.
  ...
  if ((plugin->can_read_timesteps() || plugin->can_read_qm_timestep()) &&
      ((plugin->natoms()==MOLFILE_NUMATOMS_UNKNOWN) || (plugin->natoms()>0))) {
