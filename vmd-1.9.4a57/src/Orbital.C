 ...
 *	$Revision: 1.165 $	$Date: 2022/04/09 20:53:23 $
 *
 ***************************************************************************/
/**
 * \file Orbital.C
 * \brief Orbital class manages orbitals, SCF energies, etc, for a Timestep.
 */

// Intel x86 hardware 
#if (defined(__i386__) || defined(__x86_64__) || defined(_M_IX86) || defined(_M_AMD64))
#if !defined(__SSE2__) && defined(_WIN64)
#define __SSE2__ 1      /* MSVC fails to define SSE macros */
#endif
#if defined(__SSE2__)
#include <emmintrin.h>
#define VMDORBUSESSE 1  /* build SSE code for static launch */
#endif

#if defined(VMDCPUDISPATCH)
#define VECPADSZ    16  /* max vec size is for x86 AVX512F or AVX512ER */
#else
#define VECPADSZ     4  /* fall-back to x86 SSE vector size */
#endif
...
#define VECPADSZ     4  /* IBM POWER VSX vector size */
#define VMDORBUSEVSX 1  /* build POWER VSX code for static launch */

// ARM64 SVE... 
#elif defined(__ARM_ARCH_ISA_A64) && !defined(ARCH_MACOSXARM64)
#define VMDUSESVE 1  /* build SVE code for static launch */

#if defined(VMDCPUDISPATCH)
#define VECPADSZ    16  /* max vec size is for Fujitsu A64fx */
#else
#define VECPADSZ     4  /* fall-back to NEON vector size */
#endif

// generic scalar C++ code
#else
#define VECPADSZ     1  /* scalar code, no padding */
#endif

// padding mask determined from hardware-specific 
...
//
// fctn prototypes for CPU runtime dispatch kernels
//

// AVX-512ER implementation for Xeon Phi w/ special fctn units
extern int evaluate_grid_avx512er(int numatoms,
...
// AVX-512F implementation for CPUs without exponent/reciprocal support
extern int evaluate_grid_avx512f(int numatoms,
                          const float *wave_f, const float *basis_array,
                          const float *atompos,
                          const int *atom_basis,
                          const int *num_shells_per_atom,
...
// AVX2 implementation 
extern int evaluate_grid_avx2(int numatoms,
                          const float *wave_f, const float *basis_array,
                          const float *atompos,
                          const int *atom_basis,
                          const int *num_shells_per_atom,
...
extern int evaluate_grid_neon(int numatoms,
                          const float *wave_f, const float *basis_array,
                          const float *atompos,
                          const int *atom_basis,
                          const int *num_shells_per_atom,
...
extern int evaluate_grid_sve(int numatoms,
                          const float *wave_f, const float *basis_array,
                          const float *atompos,
                          const int *atom_basis,
                          const int *num_shells_per_atom,
...
int Orbital::calculate_mo(DrawMolecule *mol, int density) {
  ...
  //
  int vecpadmask = VECPADMASK;
  if ((mol->app->cpucaps != NULL) && (mol->app->cpucaps->flags & CPU_AVX2)) {
    vecpadmask = 7; // AVX2 kernels pad to multiples of 8 
  }
  if ((mol->app->cpucaps != NULL) && (mol->app->cpucaps->flags & (CPU_AVX512F | CPU_AVX512ER))) {
    vecpadmask = 15; // AVX512 kernels pad to multiples of 16
  }

  // pad the grid X dimension for vector-multiple length and memory alignment 
...
#endif
#endif
  if (rc!=0)  rc = evaluate_grid_fast(mol->app->cpucaps,
#if defined(VMDORBUSETHRPOOL)
                                      mol->cpu_threadpool(), 
#else
                                      numcputhreads,
...
#if 0 && !defined(_WIN64) // MSVC doesn't support old MMX intrinsics
//
...
#endif // MSVC doesn't support old MMX intrinsics
...
int evaluate_grid_sse(int numatoms,
  ...
  int nx, ny, nz;
  __align(16) float sxdelta[4]; // 16-byte aligned for SSE
  for (nx=0; nx<4; nx++) 
...
#if defined(VMDCPUDISPATCH)
  wkf_cpu_caps_t *cpucaps = parms->cpucaps;

#if defined(VMDUSEAVX512)
  int dispatch_AVX512ER = 0;
  if ((cpucaps->flags & CPU_AVX512ER) && (getenv("VMDNOAVX512ER") == NULL)) {
//    printf("evaluate_grid_avx512er\n");
    dispatch_AVX512ER = 1;
  }

  int dispatch_AVX512F = 0;
  if ((cpucaps->flags & CPU_AVX512F) && (getenv("VMDNOAVX512F") == NULL)) {
    dispatch_AVX512F = 1;
//    printf("evaluate_grid_avx512f\n");
  }
#endif

#if defined(VMDUSEAVX2)
  int dispatch_AVX2 = 0;
  if ((cpucaps->flags & CPU_AVX2) && (getenv("VMDNOAVX2") == NULL)) {
    dispatch_AVX2 = 1;
//    printf("evaluate_grid_avx2\n");
  }
#endif

#if defined(VMDUSESVE)
  int dispatch_SVE = 0;
  if ((cpucaps->flags & CPU_ARM64_SVE) && (getenv("VMDNOSVE") == NULL)) {
    dispatch_SVE = 1;
//  printf("evaluate_grid_sve\n");
  }
#endif

#if defined(VMDUSENEON)
  int dispatch_NEON = 0;
  if ((cpucaps->flags & CPU_ARM64_ASIMD) && (getenv("VMDNONEON") == NULL)) {
    dispatch_NEON = 1;
//  printf("evaluate_grid_neon\n");
  }
#endif

#endif // VMDCPUDISPATCH

  // 
  // Hard-coded compile-time fall-through vectorization paths
  //
#if defined(VMDORBUSESSE) && defined(__SSE2__)
  int dispatch_SSE2 = 0;
  if ((getenv("VMDNOSSE2") == NULL)) {
//  printf("evaluate_grid_sse\n");
    dispatch_SSE2 = 1;
  }
#endif
...
#if defined(VMDORBUSEVSX) && defined(__VSX__)
  int dispatch_VSX = 0;
  if (getenv("VMDNOVSX") == NULL) {
//  printf("evaluate_grid_vsx\n");
    dispatch_VSX = 1;
  }
#endif
  ...
  while (wkf_threadlaunch_next_tile(voidparms, 1, &tile) != WKF_SCHED_DONE) {
    ...
    for (k=tile.start; k<tile.end; k++) {
      ...
      //   check for optional vector instructions and execute custom kernels 
      //   for the fastest code path supported by the detected hardware
      // 
      if (cpucaps != NULL) {
#if defined(VMDUSEAVX512)
        if (dispatch_AVX512ER) {
          evaluate_grid_avx512er(parms->numatoms, parms->wave_f, 
              parms->basis_array, parms->atompos, parms->atom_basis,
              parms->num_shells_per_atom, parms->num_prim_per_shell,
              parms->shell_types, numvoxels, parms->voxelsize,
              origin, parms->density, parms->orbitalgrid + planesize*k);
          continue;
        }

        if (dispatch_AVX512F) {
          evaluate_grid_avx512f(parms->numatoms, parms->wave_f, 
              parms->basis_array, parms->atompos, parms->atom_basis,
              parms->num_shells_per_atom, parms->num_prim_per_shell,
              parms->shell_types, numvoxels, parms->voxelsize,
              origin, parms->density, parms->orbitalgrid + planesize*k);
...
#if defined(VMDUSEAVX2)
        if (dispatch_AVX2) {
          evaluate_grid_avx2(parms->numatoms, parms->wave_f, 
              parms->basis_array, parms->atompos, parms->atom_basis,
              parms->num_shells_per_atom, parms->num_prim_per_shell,
              parms->shell_types, numvoxels, parms->voxelsize,
              origin, parms->density, parms->orbitalgrid + planesize*k);
...
#if defined(VMDUSESVE)
        if (dispatch_SVE) {
          evaluate_grid_sve(parms->numatoms, parms->wave_f,
              parms->basis_array, parms->atompos, parms->atom_basis,
              parms->num_shells_per_atom, parms->num_prim_per_shell,
              parms->shell_types, numvoxels, parms->voxelsize,
              origin, parms->density, parms->orbitalgrid + planesize*k);
...
#if defined(VMDUSENEON)
        if (dispatch_NEON) {
          evaluate_grid_neon(parms->numatoms, parms->wave_f, 
              parms->basis_array, parms->atompos, parms->atom_basis,
              parms->num_shells_per_atom, parms->num_prim_per_shell,
              parms->shell_types, numvoxels, parms->voxelsize,
              origin, parms->density, parms->orbitalgrid + planesize*k);
      ...
      } // runtime cpucaps-based dispatch
#endif


//
// hard-coded fall-through path if runtime CPU dispatch doesn't match up
//
#if defined(VMDORBUSESSE) && defined(__SSE2__)
      if (dispatch_SSE2) {
        evaluate_grid_sse(parms->numatoms, parms->wave_f, 
            parms->basis_array, parms->atompos, parms->atom_basis,
            parms->num_shells_per_atom, parms->num_prim_per_shell,
            parms->shell_types, numvoxels, parms->voxelsize,
            origin, parms->density, parms->orbitalgrid + planesize*k);
...
#if defined(VMDORBUSEVSX) && defined(__VSX__)
      if (dispatch_VSX) {
        evaluate_grid_vsx(parms->numatoms, parms->wave_f,
            parms->basis_array, parms->atompos, parms->atom_basis,
            parms->num_shells_per_atom, parms->num_prim_per_shell,
            parms->shell_types, numvoxels, parms->voxelsize,
            origin, parms->density, parms->orbitalgrid + planesize*k);
      ...
      // Standard C++-based implementation that uses wither the
      // standard math library expf(), a faster modified Cephes expf(),
      // our our own fast exponential approximation routine.
      evaluate_grid(parms->numatoms, parms->wave_f, 
            parms->basis_array, parms->atompos, parms->atom_basis,
            parms->num_shells_per_atom, parms->num_prim_per_shell,
            parms->shell_types, numvoxels, parms->voxelsize,
            origin, parms->density, parms->orbitalgrid + planesize*k);
