 ...
 *	$Revision: 1.277 $	$Date: 2020/10/21 19:11:08 $
 *
 ***************************************************************************/
/**
 *  \file BaseMolecule.C
 *  \brief Base class for all molecules, without display-specific information.
 * 
 *  This portion of a molecule contains the structural data, and all  
 *  routines to find the structure (backbone, residues, etc).
 *  It does NOT contain Timestep array; that is maintained by 
 *  DrawMolecule (which is derived from this class).
 *
 */
...
float BaseMolecule::default_mass(const char *nm) {
  ...
  if (*nm != '\0') {
    char c0 = toupper(nm[0]);
    char c1 = toupper(nm[1]); // might be '\0'...
    switch(c0) {
      ...
      case 'I': val = 126.90447f; break;                                    
      case 'K': val = 39.09830f; break;       
...
void BaseMolecule::add_volume_data(const char *name, const float *o,
  ...
  msgInfo << "   Grid size: " << x << "x" << y << "x" << z << "  (" 
          << (int) (1.0 * (vdata->gridsize() * sizeof(float)) / (1024.0 * 1024.0)) << " MB)" 
          << sendmsg;
  ...
  int gmapmb = int((3.0 * (vdata->gridsize() * sizeof(float)) / (1024.0 * 1024.0)));
  if (grad) {
    // set gradients for smooth normals
    msgInfo << "   Assigning volume gradient map" 
            << "  (" << gmapmb << " MB)" << sendmsg;
    vdata->set_volume_gradient(grad);
  } else {
    if (gmapmb <= 3072) {
      // calc gradients for smooth normals
      msgInfo << "   Computing volume gradient map " 
              << "  (" << gmapmb << " MB)" << sendmsg;
      vdata->compute_volume_gradient();
    } else {
      msgInfo << "   Volume gradient map calculations deferred to on-demand" 
              << sendmsg;
    }
  }
