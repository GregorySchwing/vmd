...
#	$Revision: 1.155 $	$Date: 2021/12/02 17:16:04 $
#
############################################################################
# DESCRIPTION:
    ...
    default {
      if ([file writable "/usr/tmp"]) {
        ...
        if { [parallel noderank] == 0 } {
          puts "***"
          puts "*** WARNING:"
          puts "*** TMPDIR env variable is unset and standard directories are unwritable!"
          puts "*** Set TMPDIR appropriately and restart VMD"
          puts "***"
        }
        set env(TMPDIR) ""
      } 
