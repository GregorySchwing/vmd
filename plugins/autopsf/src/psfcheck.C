...
PsfRes* addTopRes(char* line, FILE* psfin, FILE* outpsf) {
  ...
  while((strncmp(atomline, "ATOM", 4)==0) || 
              (strncmp(atomline, "GROU", 4)==0) || 
              (strncmp(atomline," ",1)==0) || 
              (strncmp(atomline,"!",1)==0) || 
              (strncmp(atomline,"\n",1)==0)) {
...
//    stderr << "Warning: Couldn't open input file " << topin << endl;
  ...
  while(fgets(line,100,pdbin)) {
    if ((strncmp(line, "ATOM", 4)==0) || (strncmp(line, "HETATM", 6)==0)) {
      ...
      if (knownRes->lookForRes(pdbatom)) { 
        continue;
      }
    ...
    if (strncmp(line, "CONECT", 6)==0) {
      ...
      if (res0 != NULL) {
        ...
        for (i=0; i<numbonds; i++) {
          ...
          if (strcmp(res0->name(),res1->name())==0 && 
              psfatom0->resid()==psfatom1->resid()) {
            // Both atoms belong to the same unknown residue, 
            // we can safely add a bond to res0
            ...
            printf("LINK %s:%s to %s:%s\n", 
                   psfatom0->res(), psfatom0->name(), 
                   psfatom1->res(), psfatom1->name());
